...
- [v1.0.22 のリリースノートを見る](https://github.com/domidoremi/IsleMind/releases/tag/v1.0.22)
...
`v1.0.22` のリリースには APK が含まれないため、インストール可能な最新ビルドは `v1.0.21` です。

### v1.0.22 の更新

- チャットワークスペースとモバイルナビゲーションを再構成し、メッセージリストのスクロール、キーボード操作、フローティング入力、連続した会話の流れを改善しました。
- テーマ表現とガラス表面コンポーネントを拡充し、チャット・設定・フィードバック状態の視認性を各テーマで統一しました。
- エラーメッセージがアプリで選択した言語に従って表示されます。
- プロバイダーランタイムを強化し、モデル能力の判定、リクエストパラメーターの対応付け、リクエスト最適化、ヘルスチェック、フェイルオーバー、ローカルルーティングを改善しました。
- ポータブルバックアップと復元の流れを改善し、使用量統計、価格カタログ、使用量帰属の診断機能を拡張しました。
- Expo / React Native の依存関係と Android ビルドツールチェーンを更新し、16 KB ページ互換性と ARM64 ビルド検証を継続しています。
