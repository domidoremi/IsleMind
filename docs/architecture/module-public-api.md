...
Run/task recovery honors durably acknowledged cancellation before interrupted disposition, and competing attempts re-read the durable winner rather than overwrite it. Bootstrap gates Chat admission on completed run/task recovery. Message reconstruction awaits persistence after projection; completion of a run and completion of its UI projection are not one atomic event.
