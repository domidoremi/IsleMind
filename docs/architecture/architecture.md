...
- Multilingual MiniLM remains **EXPERIMENTAL / OPT-IN**. On 2026-09-19, fresh-provider admission with retired resources, verified downloaded files and optimized Hermes measured 3,858.97 / 2,395.42 / 2,555.06 ms on M2007J3SC Android 12, meeting the unchanged ≤5,000 ms target. Tokenizer construction dominates (1,916–3,288 ms); catalogue/integrity takes 268–341 ms, tokenizer read 202–223 ms and runtime import 6–10 ms. Downloads are excluded; first native embedding is measured separately (296–332 ms). The historical 5,950 ms debug-emulator result is retained, not a same-hardware before/after optimization claim. Automatic fallback never verifies or selects experimental models, even if downloaded or bundled; it uses a verified standard model or no local model. Explicit model-and-source selection is required, and a single-device timing pass does not imply production qualification.
...
- Web remote-compaction continuation uses the real Expo SQLite/OPFS adapter, not the no-op fallback. Atomic writes share the per-database queue; identity includes conversation, provider, model and response, preventing cross-scope replacement. Reuse requires a valid completed strategy/capability envelope and bounded fragment identities. Corrupt/legacy-incompatible states are not reused; failed writes preserve the prior acknowledged state. Reads/writes emit allowlisted `context.compact.decided` storage diagnostics on failure, and Settings always exposes the browser-local/private-mode limitation. Remote compaction success is separate from persistence success. Browser exit/reload, renderer crash, corruption, failed writes and exclusive-tab retry were exercised against the real app. A persistent Chromium profile retained byte-identical OPFS files for 30 minutes; a nonpersistent context lost both SQLite files and an unrelated OPFS control file. A separate no-application private context retained its file for 30 minutes, so the cause of the loss remains unresolved, not proven browser-only. Browser eviction/clearing is not recoverable application durability, and the broad Web reliability gate remains NOT PASS.
...
`scripts/collect-native-availability-evidence.js` requires an explicit ADB executable/serial and the authorized M2007J3SC. APK inspection precedes opt-in installation. The original `com.islemind.app` is never installed, launched, stopped or cleared. Strict C4 collection still requires unchanged original private-file hashes; inability to read production storage is a blocker, never an empty successful snapshot.
...
`--mode lifecycle` requires an already installed, hash-matching qualification APK. It boots the actual ExpoRoot and application persistence owners, verifies acknowledged conversation/document/continuation data through background/foreground and process force-stop/relaunch, and checks interrupted/cancelled run recovery and terminal-journal idempotency across three more starts. WAL/FULL/foreign-key and integrity assertions remain mandatory. The 2026-09-19 physical-device run passed this scope; E4 remains locally limited, not production-signed, cross-OEM or physical-power-loss qualification. Admission timing hooks are applied only to the disposable provider source; SDK exports and production code are never monkey-patched.

The current C4 measurement run retained a 509.57 ms first-page p95 against the 500 ms limit. Five independent follow-up rounds measured 375.77–409.03 ms without code changes; this establishes variance, not permission to discard the failed sample. SQL, write/cleanup, startup delta, scrolling and recovery measurements passed their local limits. Full C4 remains NOT PASS, also because production private-file preservation cannot be inspected under the legitimate non-debuggable release boundary.
