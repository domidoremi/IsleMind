...
export function clampTraceText(input: string, limit: number): string {
  const max = Math.max(0, limit)
  if (input.length <= max) return input
  if (max < 32) return input.slice(0, max)
  return `${input.slice(0, Math.max(0, max - 32)).trimEnd()}\n[output truncated]`
}

export function projectProcessTrace(trace: ProcessTrace): ProcessTrace {
  ...
  return {
    ...
    content: trace.content
      ? clampTraceText(redactSensitiveText(trace.content), trace.type === 'tool' ? 1600 : 1200)
      : undefined,
    completedAt,
    durationMs: Math.max(0, trace.durationMs ?? completedAt - startedAt),
    metadata: sanitizeTraceMetadata(trace.metadata),
  }
}
