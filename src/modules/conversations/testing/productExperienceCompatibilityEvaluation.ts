...
export interface ProductExperiencePolicy {
  ...
  mediaGenerationReadinessVisible: boolean
  mediaGenerationNamedGatesVisible: boolean
  mediaGenerationDiagnosticOnly: boolean
  mediaGenerationExecutionActionHidden: boolean
...
const BASE_PRODUCT_EXPERIENCE_POLICY: ProductExperiencePolicy = {
  ...
  mediaGenerationReadinessVisible: true,
  mediaGenerationNamedGatesVisible: true,
  mediaGenerationDiagnosticOnly: true,
  mediaGenerationExecutionActionHidden: true,
...
export const PRODUCT_EXPERIENCE_COMPATIBILITY_FIXTURES: ProductExperienceFixture[] = [
  ...
  {
    id: 'diagnostic-only-media-generation-readiness',
    surface: 'chat',
    description: 'Future image/video generation readiness is inspectable through named gates, readiness counts, provider/settings diagnostics, and locked composer notices without exposing execution actions.',
    policy: {
      ...BASE_PRODUCT_EXPERIENCE_POLICY,
      progressVisible: false,
      cancellationVisible: false,
  ...
  {
    id: 'blocked-media-generation-execution-action',
    surface: 'chat',
    description: 'Image/video generation controls must stay blocked when readiness copy is missing, gates are unnamed, or composer/provider settings expose execution before adapter proof is captured.',
    policy: {
      ...BASE_PRODUCT_EXPERIENCE_POLICY,
      mediaGenerationReadinessVisible: false,
      mediaGenerationNamedGatesVisible: false,
      mediaGenerationDiagnosticOnly: false,
      mediaGenerationExecutionActionHidden: false,
    },
  },
  {
...
export function evaluateProductExperienceCompatibilityQualityGate(
  ...
  requireBlocked(byId.get('blocked-media-generation-execution-action'), failures, 'blocked-media-generation-execution-action', [
    'missing-media-generation-readiness',
    'missing-media-generation-gate-copy',
    'media-generation-not-diagnostic-only',
    'media-generation-execution-action-visible',
...
function requireReady(item: ProductExperienceDiagnostic | undefined, failures: string[]): void {
  ...
  if (!item.policy.mediaGenerationReadinessVisible) failures.push(`${item.fixtureId}:missing-media-generation-readiness`)
  if (!item.policy.mediaGenerationNamedGatesVisible) failures.push(`${item.fixtureId}:missing-media-generation-gate-copy`)
  if (!item.policy.mediaGenerationDiagnosticOnly) failures.push(`${item.fixtureId}:media-generation-not-diagnostic-only`)
  if (!item.policy.mediaGenerationExecutionActionHidden) failures.push(`${item.fixtureId}:media-generation-execution-action-visible`)
...
function collectProductExperienceFailureCodes(fixture: ProductExperienceFixture): ProductExperienceFailureCode[] {
  ...
  if (!policy.mediaGenerationReadinessVisible) failures.push('missing-media-generation-readiness')
  if (!policy.mediaGenerationNamedGatesVisible) failures.push('missing-media-generation-gate-copy')
  if (!policy.mediaGenerationDiagnosticOnly) failures.push('media-generation-not-diagnostic-only')
  if (!policy.mediaGenerationExecutionActionHidden) failures.push('media-generation-execution-action-visible')
