import type { McpConnectionStatus, McpPromptManifest, McpResourceManifest, McpToolManifest, McpToolPermission, McpTransport } from '@/types/mcpContracts'
...
export interface McpCompatibilityRuntimeLogOptions {
  enabled?: boolean
  maxBytes?: number
}

export interface McpCompatibilityRuntimeEventEnvelope {
  schema: string
  id: string
  ts: string
...
export interface McpCompatibilityRuntimeEventPort {
  emitRuntimeEvent(input: {
    event: 'tool.mcp.compatibility.checked'
    data: Record<string, unknown>
    legacyData: Record<string, unknown>
    options?: McpCompatibilityRuntimeLogOptions
  }): Promise<McpCompatibilityRuntimeEventEnvelope>
}
...
export const MCP_COMPATIBILITY_FIXTURES: McpCompatibilityFixture[] = [
  ...
  {
    id: 'streamable-http-gateway',
    serverId: 'streamable-gateway',
    serverName: 'Streamable HTTP MCP',
    serverSource: 'modelcontextprotocol/streamable-http',
    url: 'https://mcp.streamable.example.test/mcp',
    transport: 'streamable-http',
    enabledToolNames: ['session_initialize', 'tool_catalog'],
    responses: {
      initialize: {
        serverInfo: { name: 'streamable-http-fixture', version: '2025.3.26' },
      },
      'tools/list': {
        tools: [
          {
            name: 'session_initialize',
            description: 'Initialize a Streamable HTTP MCP session.',
            inputSchema: { type: 'object', properties: { clientName: { type: 'string' } } },
          },
          {
            name: 'tool_catalog',
            description: 'Read the bounded gateway tool catalog.',
      ...
      'resources/list': {
        resources: [
          { uri: 'mcp://streamable-http/session', name: 'Streamable HTTP session', mimeType: 'application/json' },
        ],
      },
      'prompts/list': {
        prompts: [
          { name: 'streamable_http_status', description: 'Summarize Streamable HTTP gateway readiness.' },
        ],
      },
    },
  },
...
export async function emitMcpCompatibilityRuntimeSummaryEvent(
  ...
  runtimeEvents: McpCompatibilityRuntimeEventPort,
  trigger = 'diagnostics-refresh',
  options?: McpCompatibilityRuntimeLogOptions
): Promise<McpCompatibilityRuntimeEventEnvelope> {
  const summary = buildMcpCompatibilityRuntimeSummary(run)
  return runtimeEvents.emitRuntimeEvent({
    event: 'tool.mcp.compatibility.checked',
    data: {
      trigger,
      ...summary,
...
export function evaluateMcpCompatibilityFixture(fixture: McpCompatibilityFixture, now = Date.now()): McpCompatibilityDiagnostic {
  ...
  if (fixture.transport !== 'sse' && fixture.transport !== 'streamable-http') {
    return {
      ...base,
      status: 'error',
      refreshResult: 'error',
...
export function evaluateMcpCompatibilityQualityGate(
  ...
  requireConnected(byId.get('streamable-http-gateway'), failures, { minTools: 2, minResources: 1, minPrompts: 1 })
