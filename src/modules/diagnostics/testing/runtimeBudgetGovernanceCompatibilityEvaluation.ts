...
export interface RuntimeBudgetGovernancePolicy {
  ...
  mediaDurationBudgetMs: number
  estimatedMediaDurationMs: number
  mediaGenerationStreamCleanupScope: typeof MEDIA_GENERATION_STREAM_CLEANUP_SCOPE
  mediaGenerationAbortControllerRequired: boolean
  mediaGenerationPartialArtifactCleanupRequired: boolean
  mediaGenerationAdapterImplemented: boolean
  mediaGenerationExecutionDisabled: boolean
...
const MEDIA_GENERATION_CANCELLATION_CLEANUP_CONTRACT = buildMediaGenerationCancellationCleanupContract()
...
const SAFE_RUNTIME_BUDGET_POLICY: RuntimeBudgetGovernancePolicy = {
  ...
  artifactByteBudget: MEDIA_GENERATION_ARTIFACT_MAX_BYTES,
  estimatedArtifactBytes: 8 * 1024 * 1024,
  mediaDurationBudgetMs: MEDIA_GENERATION_VIDEO_DURATION_MAX_MS,
  estimatedMediaDurationMs: 60_000,
  mediaGenerationStreamCleanupScope: MEDIA_GENERATION_STREAM_CLEANUP_SCOPE,
  mediaGenerationAbortControllerRequired: MEDIA_GENERATION_CANCELLATION_CLEANUP_CONTRACT.abortControllerRequired,
  mediaGenerationPartialArtifactCleanupRequired: MEDIA_GENERATION_CANCELLATION_CLEANUP_CONTRACT.partialArtifactCleanupRequired,
  mediaGenerationAdapterImplemented: MEDIA_GENERATION_ADAPTER_IMPLEMENTED,
  mediaGenerationExecutionDisabled: MEDIA_GENERATION_CANCELLATION_CLEANUP_CONTRACT.executionDisabled,
...
export const RUNTIME_BUDGET_GOVERNANCE_COMPATIBILITY_FIXTURES: RuntimeBudgetGovernanceFixture[] = [
  ...
  {
    id: 'generated-media-task-budget-governance',
    surface: 'media-generation',
    expectedReadiness: 'degraded',
    description: 'Future image/video generation tasks carry artifact byte, media duration, timeout, stream-idle, cost, cancellation cleanup, budget-ledger, and disabled-execution constraints before any adapter can run.',
    policy: {
      ...SAFE_RUNTIME_BUDGET_POLICY,
      budgetKinds: ['artifact-bytes', 'media-duration', 'timeout', 'stream-idle', 'cost', 'latency'],
      artifactByteBudget: MEDIA_GENERATION_ARTIFACT_MAX_BYTES,
      estimatedArtifactBytes: 12 * 1024 * 1024,
      mediaDurationBudgetMs: MEDIA_GENERATION_VIDEO_DURATION_MAX_MS,
      estimatedMediaDurationMs: 90_000,
      timeoutMs: 180000,
      streamIdleTimeoutMs: 10000,
      cancellationPropagated: true,
      mediaGenerationExecutionDisabled: true,
    },
  },
  {
  ...
  {
    id: 'blocked-generated-media-task-budget',
    surface: 'blocked',
    expectedReadiness: 'blocked',
    description: 'Generated media task execution is blocked when artifact bytes, media duration, cancellation cleanup, or disabled-execution invariants are missing.',
    policy: {
      ...SAFE_RUNTIME_BUDGET_POLICY,
      budgetKinds: ['artifact-bytes', 'media-duration', 'timeout', 'stream-idle'],
      artifactByteBudget: 0,
      estimatedArtifactBytes: MEDIA_GENERATION_ARTIFACT_MAX_BYTES + 1,
      mediaDurationBudgetMs: 0,
      estimatedMediaDurationMs: MEDIA_GENERATION_VIDEO_DURATION_MAX_MS + 1,
      cancellationPropagated: false,
      mediaGenerationAbortControllerRequired: false,
      mediaGenerationPartialArtifactCleanupRequired: false,
      mediaGenerationAdapterImplemented: true,
      mediaGenerationExecutionDisabled: false,
    },
  },
  {
...
export function evaluateRuntimeBudgetGovernanceCompatibilityQualityGate(
  ...
  requireDegraded(byId.get('generated-media-task-budget-governance'), failures, 'generated-media-task-budget-governance', { requireMediaGenerationPolicy: true })
  ...
  requireBlocked(byId.get('blocked-generated-media-task-budget'), failures, 'blocked-generated-media-task-budget', [
    'missing-cancellation',
    'missing-artifact-size-budget',
    'missing-media-duration-budget',
    'missing-media-cleanup-contract',
...
function requireDegraded(
  ...
  failures: string[],
  id: string,
  options: { requireFallbackVisible?: boolean; requireLocalResourcePolicy?: boolean; requireMediaGenerationPolicy?: boolean } = {},
): void {
  if (!item) return
  ...
  if (options.requireMediaGenerationPolicy && (!item.policy.mediaGenerationExecutionDisabled || item.policy.mediaGenerationAdapterImplemented || !item.policy.mediaGenerationAbortControllerRequired || !item.policy.mediaGenerationPartialArtifactCleanupRequired)) failures.push(`${id}:missing-media-generation-policy`)
...
function requireBaselineBudgetPolicy(item: RuntimeBudgetGovernanceDiagnostic, failures: string[]): void {
  ...
  if (item.policy.artifactByteBudget <= 0) failures.push(`${item.fixtureId}:missing-artifact-size-budget`)
  if (item.policy.mediaDurationBudgetMs <= 0) failures.push(`${item.fixtureId}:missing-media-duration-budget`)
  if (item.policy.mediaGenerationStreamCleanupScope !== MEDIA_GENERATION_STREAM_CLEANUP_SCOPE || !item.policy.mediaGenerationAbortControllerRequired || !item.policy.mediaGenerationPartialArtifactCleanupRequired) failures.push(`${item.fixtureId}:missing-media-cleanup-contract`)
  if (item.policy.mediaGenerationAdapterImplemented || !item.policy.mediaGenerationExecutionDisabled) failures.push(`${item.fixtureId}:media-generation-execution-enabled`)
...
function collectRuntimeBudgetGovernanceFailureCodes(fixture: RuntimeBudgetGovernanceFixture): RuntimeBudgetGovernanceFailureCode[] {
  ...
  if (policy.artifactByteBudget <= 0) failures.push('missing-artifact-size-budget')
  if (policy.mediaDurationBudgetMs <= 0) failures.push('missing-media-duration-budget')
  if (policy.mediaGenerationStreamCleanupScope !== MEDIA_GENERATION_STREAM_CLEANUP_SCOPE || !policy.mediaGenerationAbortControllerRequired || !policy.mediaGenerationPartialArtifactCleanupRequired) failures.push('missing-media-cleanup-contract')
  if (policy.mediaGenerationAdapterImplemented || !policy.mediaGenerationExecutionDisabled) failures.push('media-generation-execution-enabled')
...
function budgetExceeded(policy: RuntimeBudgetGovernancePolicy): boolean {
  return policy.estimatedInputTokens > policy.inputTokenBudget ||
    ...
    policy.estimatedArtifactBytes > policy.artifactByteBudget ||
    policy.estimatedMediaDurationMs > policy.mediaDurationBudgetMs ||
