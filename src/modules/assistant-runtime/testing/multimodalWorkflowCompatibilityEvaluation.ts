import { MEDIA_GENERATION_ARTIFACT_MANIFEST_SCHEMA } from '@/core/mediaGenerationContracts'
...
export interface MultimodalWorkflowGuardrails {
  ...
  artifactManifestContract?: string
  nativeProof: 'not-needed' | 'required-before-default' | 'captured'
  auditEvent: boolean
}
...
export const MULTIMODAL_WORKFLOW_COMPATIBILITY_FIXTURES: MultimodalWorkflowFixture[] = [
  {
    ...
    docs: ['docs/architecture/vnext-migration-status.md'],
  ...
  {
    id: 'mobile-composer-mode-gated-media',
    providerFamily: 'app-workflow',
    surface: 'provider-chat',
    docs: ['docs/architecture/islemind-vnext-architecture-refactor-plan.md'],
    description: 'Mobile composer media entries are visible but mode-aware and provider/model-gated before image, file, or voice input can enter a request.',
    inputModalities: ['text', 'image', 'file', 'audio'],
    outputModalities: ['text'],
    requestShape: 'mode-aware-composer-gate',
    modelMetadataSupportsModalities: true,
    appRequestControl: true,
    requiresAdapter: false,
    guardrails: {
  ...
  {
    id: 'media-generation-adapter-required',
    providerFamily: 'external-worker',
    surface: 'external-worker',
    docs: ['docs/architecture/islemind-vnext-architecture-refactor-plan.md'],
    description: 'Future image or video generation must stay behind an explicit adapter, provider capability evidence, artifact manifests, retention controls, cancellation semantics, and native proof.',
    inputModalities: ['text', 'image'],
    outputModalities: ['image'],
    requestShape: 'media-generation-artifact',
    modelMetadataSupportsModalities: true,
    appRequestControl: true,
    requiresAdapter: true,
    guardrails: {
      ...SAFE_MEDIA_GUARDRAILS,
      sizeByteLimit: 32 * 1024 * 1024,
      frameLimit: 16,
      externalWorkerAllowed: false,
      artifactManifest: true,
      artifactManifestContract: MEDIA_GENERATION_ARTIFACT_MANIFEST_SCHEMA,
      nativeProof: 'required-before-default',
    },
  },
  {
...
export function evaluateMultimodalWorkflowCompatibilityQualityGate(
  ...
  requireReady(byId.get('mobile-composer-mode-gated-media'), failures, 'mode-aware-composer-gate')
  ...
  requireNeedsAdapter(byId.get('media-generation-adapter-required'), failures, 'media-generation-artifact', ['adapter-required', 'external-worker-required'])
...
function requireSafeMediaPolicy(item: MultimodalWorkflowDiagnostic, failures: string[]): void {
  ...
  if (item.guardrails.nativeProof === 'required-before-default') failures.push(`${item.fixtureId}:missing-native-proof`)
...
function collectMultimodalWorkflowFailureCodes(fixture: MultimodalWorkflowFixture): MultimodalWorkflowFailureCode[] {
  ...
  if (fixture.requestShape === 'media-generation-artifact' && fixture.guardrails.artifactManifestContract !== MEDIA_GENERATION_ARTIFACT_MANIFEST_SCHEMA) failures.push('artifact-contract-missing')
  ...
  if (fixture.guardrails.nativeProof === 'required-before-default') failures.push('native-proof-missing')
