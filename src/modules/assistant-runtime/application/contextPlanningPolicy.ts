export type ContextPlanningRole = "user" | "assistant";
export type ContextPlanningRemoteCompactMode = "off" | "auto" | "required";

export interface ContextPlanningMessage<TAttachment = unknown> {
  role: ContextPlanningRole;
  content?: string | null;
...
export interface ContextPlanningRequestMessage {
  role: ContextPlanningRole;
  content: string;
}

export interface ContextPlanningCompressionSectionMetadata {
  id:
    | "constraints"
    | "decisions"
    | "failures"
...
export interface ContextPlanningCompressionRoleCounts {
  user: number;
  assistant: number;
}

export interface ContextPlanningCompressionMetadata {
  schemaVersion: 2;
  strategy:
    | "none"
    | "structured-v2"
  ...
  sourceRoleCounts: ContextPlanningCompressionRoleCounts;
  keptRoleCounts: ContextPlanningCompressionRoleCounts;
  sourceTokens: number;
  ...
  summarySections: ContextPlanningCompressionSectionMetadata[];
}

export interface ContextPlanningPackedChatMessages {
  messages: ContextPlanningRequestMessage[];
  ...
  compressionMetadata: ContextPlanningCompressionMetadata;
}

export type ContextPlanningProvider = RemoteCompactDecisionInput["provider"];

export interface ContextPlanningModelManifest {
  id: string;
  provider: string;
...
export interface ContextPlanningPackChatMessagesInput {
  messages: readonly ContextPlanningMessage[];
  contextPrompt?: string;
  modelContextWindow: number;
  maxOutputTokens: number;
  systemPrompt?: string;
  reasoningEffort?: unknown;
  provider?: ContextPlanningProvider;
  providerType?: unknown;
  model?: string;
  localCompression?: boolean;
}

export type ContextPlanningRemoteCompactDecisionInput =
  RemoteCompactDecisionInput;

export type ContextPlanningRemoteCompactDecision = RemoteCompactDecision;

export interface ContextPlanningRuntimeEventInput {
  event:
    | "context.planned"
...
export type ContextFragmentExclusionReason =
  | "empty"
  | "token_cap_exceeded"
...
export const CONTEXT_ASSEMBLY_MANIFEST_SCHEMA =
  "islemind.context-assembly-manifest.v1";
...
export interface RemoteCompactLocalFallbackPlan {
  messages: ContextPlanningPackedChatMessages["messages"];
  contextPrompt: string;
  trace: Record<string, unknown>;
}
...
export interface ContextPlan {
  contextPrompt: string;
  messages: ContextPlanningPackedChatMessages["messages"];
  packed: ContextPlanningPackedChatMessages;
  remoteCompactProbe: ContextPlanningPackedChatMessages;
  localFallback: ContextPlanningPackedChatMessages;
  remoteCompactFallback?: RemoteCompactLocalFallbackPlan;
  compactDecision: ContextPlanningRemoteCompactDecision;
...
export interface ContextPlannerInput {
  ...
  modelContextWindow: number;
  maxOutputTokens: number;
  modelManifest?: ContextPlanningModelManifest;
  systemPrompt?: string;
  reasoningEffort?: unknown;
  provider: ContextPlanningProvider;
  providerType?: unknown;
  ...
  settings?: Pick<
    ...
    | "runtimeLogMaxBytes"
  >;
  retrievalSources?: readonly unknown[];
...
export function buildContextPlannerPrompt(
  input: Pick<ContextPlannerInput, "baseContextPrompt" | "contextSources">,
): string | undefined {
...
export interface ContextPlanningPolicyDependencies {
  packChatMessages(
    input: ContextPlanningPackChatMessagesInput,
  ): ContextPlanningPackedChatMessages;
  decideRemoteCompact(
    input: ContextPlanningRemoteCompactDecisionInput,
  ): ContextPlanningRemoteCompactDecision;
  estimateTextTokens(text: string): number;
  emitRuntimeEvent(input: ContextPlanningRuntimeEventInput): unknown;
}

export interface ContextPlanningPolicy {
  planChatContext(input: ContextPlannerInput): ContextPlan;
  buildContextPlannerPrompt: typeof buildContextPlannerPrompt;
}

export function createContextPlanningPolicy(
  dependencies: ContextPlanningPolicyDependencies,
): ContextPlanningPolicy {
  function planChatContext(input: ContextPlannerInput): ContextPlan {
    ...
    const compactDecision = dependencies.decideRemoteCompact({
    ...
    const blocksForMissingRequiredRemote =
      compactDecision.required && !compactDecision.supported;
    // Native server compact: send full untrimmed history (probe).
    // Application / client shrink: send structured localFallback.
    // Off: preserve historical bounded local compression.
    // Auto below threshold: send full probe history without forced local compression.
    let packed = remoteCompactProbe;
    if (blocksForMissingRequiredRemote) {
      packed = remoteCompactProbe;
    } else if (compactDecision.mode === "off") {
      packed = localFallback;
    } else if (compactDecision.enabled && compactDecision.nativeServerCompact) {
      packed = remoteCompactProbe;
    } else if (compactDecision.enabled) {
      packed = localFallback;
    }
    const remoteCompactFallback =
      compactDecision.enabled && compactDecision.nativeServerCompact
    ...
    const baseFragments = buildFragments(
      plannerInput,
      packed,
      compactDecision,
      windowState,
    ...
    const cacheDiagnostics = buildContextCacheDiagnostics(
      baseFragments,
      input.previousFragments,
    );
    const fragments = annotateContextFragmentCache(
      baseFragments,
      input.previousFragments,
    );
    ...
    return {
      ...
      trace: {
        ...
        includedContextSourceCount: plannedContextSources.filter(
          (source) => source.included,
        ).length,
        cappedContextSourceCount: plannedContextSources.filter(
          (source) => source.capped,
        ).length,
        excludedContextSourceCount: plannedContextSources.filter(
          (source) => !source.included,
        ).length,
        cacheDiagnosticCount: cacheDiagnostics.length,
        cacheDiagnostics,
        contextManifest: summarizeContextAssemblyManifest(manifest),
        draftTokens: input.draft?.text
          ? dependencies.estimateTextTokens(input.draft.text)
          : undefined,
  ...
  function pack(
    ...
    return dependencies.packChatMessages({
  ...
  function buildRemoteCompactFallback(
    localFallback: ContextPlanningPackedChatMessages,
  ): RemoteCompactLocalFallbackPlan {
    return {
      ...
      trace: {
        ...
        compressionTriggerReason:
          localFallback.compressionMetadata.triggerReason,
        estimatedSavedTokens:
          localFallback.compressionMetadata.estimatedSavedTokens,
  ...
  function buildContextWindowState(input: {
    ...
    const autoCompactScopeTokens = estimatePackedInputTokens(
      input.remoteCompactProbe,
    );
    const tokensUntilCompaction = Math.max(
      0,
      input.packed.budgetTokens - input.packed.estimatedInputTokens,
    );
    return {
      activeContextTokens,
      ...
      previousResponseId: input.previousResponseId,
      lastCompactSummary: input.packed.compressionTriggered
        ? input.packed.contextPrompt
        : undefined,
      compactFailureState:
        input.compactDecision.required && !input.compactDecision.supported
  ...
  function buildFragments(
    ...
    if (input.systemPrompt) {
      fragments.push(
        fragment({
          ...
          estimatedTokens: dependencies.estimateTextTokens(input.systemPrompt),
          originalEstimatedTokens: dependencies.estimateTextTokens(
            input.systemPrompt,
          ),
          included: true,
          capped: false,
      ...
      fragments.push(
        fragment({
          ...
          trace: {
            sourceCount:
              source.sourceCount ??
              (source.type === "retrieved_context"
                ? (input.retrievalSources?.length ?? 0)
                : undefined),
            capped: source.capped,
            originalEstimatedTokens: source.originalEstimatedTokens,
            ...source.trace,
    ...
    fragments.push(
      fragment({
        ...
        originalEstimatedTokens:
          packed.messageTokens + packed.compressionMetadata.sourceTokens,
        included: true,
        capped: packed.truncatedSingleMessage,
        exclusionReason: packed.truncatedSingleMessage
          ? "token_cap_exceeded"
          : undefined,
        trace: {
    ...
    if (
      ...
      windowState.compactFailureState
    ) {
      fragments.push(
        fragment({
          ...
          estimatedTokens: dependencies.estimateTextTokens(
            [input.previousResponseId, windowState.lastCompactSummary]
              .filter(Boolean)
              .join("\n"),
          ),
          originalEstimatedTokens: dependencies.estimateTextTokens(
  ...
  function fragment(input: {
    ...
    const sourceHash =
      input.sourceHash ??
      stableContextSourceHash(
        `${input.type}:${input.sourceId}:${input.text ?? ""}`,
      );
    return {
      ...
      originalEstimatedTokens: Math.max(
        0,
        Math.floor(input.originalEstimatedTokens),
      ),
  ...
  function estimatePackedInputTokens(
    packed: ContextPlanningPackedChatMessages,
  ): number {
  ...
  function emitContextPlannerEvents(
    ...
    const includedFragments = plan.fragments.filter(
      (fragment) => fragment.included,
    );
    const excludedFragments = plan.fragments.filter(
      (fragment) => !fragment.included,
    );
    void dependencies.emitRuntimeEvent({
      ...
      data: {
        ...
        cappedFragmentCount: plan.fragments.filter(
          (fragment) => fragment.capped,
        ).length,
        contextSourceCount: plan.plannedContextSources.length,
        messageCount: plan.messages.length,
        contextPromptTokens: dependencies.estimateTextTokens(
          plan.contextPrompt,
        ),
        ...
        cacheDiagnostics: summarizeContextCacheDiagnostics(
          plan.cacheDiagnostics,
        ),
  ...
  function buildContextCacheDiagnostics(
    fragments: ContextFragment[],
    previousFragments: readonly PreviousContextFragmentIdentity[] | undefined,
    ...
    const previousIncluded = (previousFragments ?? []).filter(
      (fragment) => fragment.included !== false,
    );
    if (!previousIncluded.length) return diagnostics;

    const previousBySourceId = new Map(
      previousIncluded.map((fragment) => [fragment.sourceId, fragment]),
    );
    const previousBySourceHash = new Map(
      previousIncluded
        .filter((fragment) => fragment.sourceHash)
        .map((fragment) => [fragment.sourceHash!, fragment]),
    );
    let checkedCount = 0;
    let changedCount = 0;
    for (const fragment of includedFragments) {
      ...
      if (
        previousBySource?.sourceHash &&
        fragment.sourceHash &&
        previousBySource.sourceHash !== fragment.sourceHash
      ) {
        checkedCount += 1;
        changedCount += 1;
        diagnostics.push({
      ...
      const previousByHash = fragment.sourceHash
        ? previousBySourceHash.get(fragment.sourceHash)
        : undefined;
      if (
        previousByHash &&
        (previousByHash.id !== fragment.id ||
          previousByHash.sourceId !== fragment.sourceId)
      ) {
        diagnostics.push({
  ...
  function annotateContextFragmentCache(
    ...
    return fragments.map((fragment) => {
      ...
      const previousByHash = fragment.sourceHash
        ? previousBySourceHash.get(fragment.sourceHash)
        : undefined;
      const reuseHint = contextFragmentReuseHint(
        fragment,
        previousBySource,
        previousByHash,
      );
  ...
  function contextFragmentReuseHint(
    ...
    if (!Number.isFinite(fragment.tokenCap) || fragment.tokenCap <= 0)
      return "unbounded_fragment_blocked";
    ...
    if (
      ...
      return "fragment_id_changed";
    return "stable_source_hash";
  }

  function summarizeContextCacheDiagnostics(
    diagnostics: ContextCacheDiagnostic[],
  ): ContextCacheDiagnostic[] {
  ...
  function buildContextAssemblyManifest(
    ...
    const manifestFragments = plan.fragments.map((fragment) =>
      buildContextManifestFragment(fragment, plan.packed.budgetTokens),
    );
    const failureCodes = contextManifestFailureCodes(
      plan.fragments,
      plan.cacheDiagnostics,
      plan.windowState,
      input,
    ...
    const cappedFragments = plan.fragments.filter(
      (fragment) => fragment.capped,
    );
    const totalTokenCap = plan.fragments.reduce(
      (sum, fragment) => sum + fragment.tokenCap,
      0,
    );
    const originalFragmentTokens = plan.fragments.reduce(
      (sum, fragment) => sum + fragment.originalEstimatedTokens,
      0,
    );
    const includedFragmentTokens = includedFragments.reduce(
    ...
    return {
      ...
      counts: {
        ...
        retrievedContextFragments: plan.fragments.filter(
          (fragment) => fragment.type === "retrieved_context",
        ).length,
        memoryFragments: plan.fragments.filter(
          (fragment) => fragment.type === "memory",
        ).length,
        toolOutputFragments: plan.fragments.filter(
          (fragment) => fragment.type === "tool_outputs",
        ).length,
        authorities,
      },
      guardrails: {
        rawTextSerialized: false,
        networkCallsAllowed: false,
        modelVisibleContextRequiresFiniteCap: includedFragments.every(
          (fragment) =>
            Number.isFinite(fragment.tokenCap) && fragment.tokenCap > 0,
        ),
        modelVisibleSourcesRequireHash: includedFragments.every((fragment) =>
          Boolean(fragment.sourceHash),
        ),
        excludedSourcesRequireReason: excludedFragments.every((fragment) =>
          Boolean(fragment.exclusionReason),
        ),
      },
      failureCodes,
  ...
  function buildContextManifestFragment(
    fragment: ContextFragment,
    requestBudgetTokens: number,
  ): ContextAssemblyManifestFragment {
    return {
      ...
      estimatedTokens: fragment.estimatedTokens,
      originalEstimatedTokens: fragment.originalEstimatedTokens,
      budgetShare: roundContextManifestRatio(
  ...
  function summarizeContextAssemblyManifest(
    manifest: ContextAssemblyManifest,
  ): Pick<
    ContextAssemblyManifest,
  ...
  function contextManifestFailureCodes(
    ...
    for (const fragment of fragments) {
      if (fragment.exclusionReason === "empty")
        codes.add("context_source_empty");
      if (fragment.exclusionReason === "unbounded_fragment_blocked")
        codes.add("context_source_unbounded");
      if (fragment.capped || fragment.exclusionReason === "token_cap_exceeded")
        codes.add("context_source_token_cap_exceeded");
      if (fragment.included && !fragment.sourceHash)
        codes.add("context_source_missing_hash");
      if (
        fragment.included &&
        (!Number.isFinite(fragment.tokenCap) ||
          fragment.tokenCap <= 0 ||
          fragment.estimatedTokens > fragment.tokenCap)
      ) {
    ...
    if (
      ...
      windowState.activeContextTokens > input.modelContextWindow
    ) {
      codes.add("context_budget_overrun");
    }
    for (const diagnostic of cacheDiagnostics) {
      if (diagnostic.kind === "source_hash_changed")
        codes.add("context_source_hash_changed");
      if (diagnostic.kind === "fragment_id_changed_same_source_hash")
        codes.add("context_fragment_id_churn");
      if (diagnostic.kind === "full_context_rewrite_detected")
        codes.add("context_full_rewrite_detected");
      if (diagnostic.kind === "unbounded_fragment_blocked")
        codes.add("context_source_unbounded");
    }
  ...
  function contextManifestAuthority(
    fragment: ContextFragment,
  ): ContextManifestAuthority {
    ...
    if (
      fragment.type === "retrieved_context" ||
      fragment.type === "memory" ||
      fragment.type === "attachments"
    )
      return "user-private";
    if (fragment.type === "tool_outputs") return "permissioned-tool";
    if (
      fragment.type === "history_summary" ||
      fragment.type === "recent_messages"
    )
      return "conversation";
  ...
  function contextManifestDecision(
    fragment: ContextFragment,
  ...
  function contextManifestReliability(
    fragment: ContextFragment,
  ): ContextManifestReliability {
    if (fragment.exclusionReason === "unbounded_fragment_blocked")
    ...
    if (
      fragment.type === "history_summary" ||
      fragment.type === "recent_messages" ||
      fragment.type === "remote_compact_state"
    ) {
      return "derived_runtime_state";
    }
    if (fragment.cache.reuseHint === "source_hash_reused")
      return "stable_reused_hash";
    if (fragment.capped) return "bounded_capped_source";
    return "bounded_source_hash";
  }

  function contextManifestSourceCount(
    fragment: ContextFragment,
  ): number | undefined {
  ...
  function emptyContextManifestAuthorityCounts(): Record<
    ContextManifestAuthority,
    number
  > {
    return {
  ...
  function runtimeLogOptionsForContextPlanner(input: ContextPlannerInput): {
    enabled?: boolean;
    maxBytes?: number;
  } {
    return {
  ...
  function planContextSources(
    ...
    return sources.map((source) => {
      ...
      const originalEstimatedTokens =
        dependencies.estimateTextTokens(originalText);
  ...
  function contextFragmentTokenCap(
    type: ContextFragmentType,
    input: Pick<ContextPlannerInput, "modelContextWindow" | "maxOutputTokens">,
  ): number {
    if (
      !Number.isFinite(input.modelContextWindow) ||
    ...
    const usableWindow = Math.max(
      256,
      input.modelContextWindow - input.maxOutputTokens,
    );
    const ratio =
      type === "tool_outputs"
        ? 0.14
  ...
  function clampContextSourceText(text: string, tokenCap: number): string {
    ...
    let next = text.trim();
    while (
      dependencies.estimateTextTokens(next) > tokenCap &&
      next.length > 180
    ) {
      next = preserveContextHeadTail(
        next,
    ...
    while (
      dependencies.estimateTextTokens(withPrefix) > tokenCap &&
      next.length > 120
    ) {
      next = preserveContextHeadTail(
        next,
        Math.max(96, Math.floor(next.length * 0.84)),
      );
      withPrefix = `${prefix}${next}`;
    }
    return dependencies.estimateTextTokens(withPrefix) <= tokenCap
      ? withPrefix
      : next;
  }
