...
export interface ProviderTextToolCallStreamFilter {
...
export function extractProviderToolCalls(value: unknown, providerType: string): ProviderToolCall[] | undefined {
...
export function createProviderTextToolCallStreamFilter(): ProviderTextToolCallStreamFilter {
  let buffer = ''
...
function collectOpenAIProviderToolCalls(root: Record<string, unknown>, add: (input: ProviderToolCallInput) => void) {
  ...
  if (isToolEventType(root.type) || root.name || root.arguments || root.input) addOpenAIToolCall(root)
  ...
  for (const output of Array.isArray(root.output) ? root.output : []) {
    ...
    if (!item) continue
    if (isToolEventType(item.type) || item.name || item.arguments || item.input) addOpenAIToolCall(item)
...
function collectAnthropicProviderToolCalls(root: Record<string, unknown>, add: (input: ProviderToolCallInput) => void) {
  ...
  if (root.type === 'content_block_delta' && delta?.type === 'input_json_delta') {
    add({ index, rawArguments: delta.partial_json })
  }
}
