...
import { asRecord, safeJsonPreview, stringValue, stringifyReasoningDetails } from './providerJsonPolicy'
import { extractOpenAIContentReasoning, stringifyOpenAIReasoningItem } from './providerResponseText'

export interface ProviderTraceExtractionOptions {
  includeReasoning?: boolean
}

export interface ProviderToolTraceMetadataInput {
...
export interface ProviderTracePolicyDependencies {
  translate(key: string, values?: Record<string, unknown>): string
  buildToolCallMetadata(input: ProviderToolTraceMetadataInput): Record<string, unknown>
  inferToolName(title: string, content: string): string
  redact(content: string): string
  now?: () => number
}

export function createProviderTracePolicy(dependencies: ProviderTracePolicyDependencies) {
  ...
  function summarizeToolEvent(value: unknown): string {
    ...
    return [
      name ? dependencies.translate('providerTrace.toolNameLine', { name }) : '',
      inputText ? dependencies.translate('providerTrace.toolArgsLine', { input: inputText }) : '',
    ].filter(Boolean).join('\n')
  }
  ...
  function createProviderTrace(
    ...
    const toolCallMetadata = type === 'tool'
      ? dependencies.buildToolCallMetadata({
          mode: 'native-provider',
          source: 'provider',
          toolName: dependencies.inferToolName(title, content),
          status,
          providerType,
        })
      : {}
    return {
      ...
      type,
      title,
      content: sanitizeTraceContent(content, dependencies.redact),
      status,
      startedAt: timestamp,
      completedAt: status === 'done' || status === 'error' || status === 'skipped' ? timestamp : undefined,
      metadata: {
        providerType,
        source: 'provider',
        ...toolCallMetadata,
  ...
  function extractTracesFromJson(
    json: any,
    providerType: ProviderType,
    options: ProviderTraceExtractionOptions = {},
  ): ProcessTrace[] {
    ...
    if (providerType === 'openai' || providerType === 'openai-compatible' || providerType === 'xiaomi-mimo') {
      if (includeReasoning) {
        ...
        if (reasoning) {
          traces.push(createProviderTrace(
            'reasoning',
            providerType,
            dependencies.translate('providerTrace.reasoningSummary'),
            reasoning,
            'done',
            stableTraceId(json, 'reasoning-json'),
          ))
        }
      }
        ...
        for (const item of json.output) {
          ...
          if (isToolEventType(record.type)) {
            traces.push(createProviderTrace(
              ...
              summarizeToolEvent(record),
              'done',
              stableTraceId(record, 'tool-json'),
            ))
          }
        }
      }
    ...
    if (providerType === 'anthropic' && Array.isArray(json.content)) {
      for (const part of json.content) {
        ...
        if (includeReasoning && item.type === 'thinking') {
          traces.push(createProviderTrace(
            'reasoning',
            providerType,
            dependencies.translate('providerTrace.reasoningSummary'),
            ...
            'done',
            stableTraceId(item, 'thinking-json'),
          ))
        }
        if (item.type === 'tool_use') {
          traces.push(createProviderTrace(
            'tool',
            providerType,
            dependencies.translate('providerTrace.toolCallNamed', { name: stringValue(item.name) || 'tool' }),
            summarizeToolEvent(item),
            'done',
            stableTraceId(item, 'tool-json'),
          ))
        }
      }
    }
    if (providerType === 'google') {
      ...
      if (Array.isArray(parts)) {
        for (const part of parts) {
          if (includeReasoning && part.thought && part.text) {
            traces.push(createProviderTrace(
              'reasoning',
              providerType,
              dependencies.translate('providerTrace.reasoningSummary'),
              ...
              'done',
              stableTraceId(part, 'thought-json'),
            ))
          }
          if (part.functionCall) {
            traces.push(createProviderTrace(
              'tool',
              providerType,
              dependencies.translate('providerTrace.functionCallNamed', { name: part.functionCall.name ?? 'function' }),
              summarizeToolEvent(part.functionCall),
              'done',
              stableTraceId(part.functionCall, 'function-json'),
            ))
          }
          if (includeReasoning && part.thoughtSignature) {
            traces.push(createProviderTrace(
              'reasoning',
              providerType,
              ...
              'done',
              stableTraceId(part, 'thought-signature-json'),
    ...
    return dedupeProviderTraces(traces)
  }
...
export function isReasoningEventType(type: unknown): boolean {
  return typeof type === 'string' &&
    (type.includes('reasoning') || type.includes('thinking') || type.includes('summary'))
}

export function isToolEventType(type: unknown): boolean {
  return typeof type === 'string' &&
    (type.includes('tool') || type.includes('function_call') || type.includes('web_search'))
}

export function isDoneEvent(type: unknown): boolean {
  return typeof type === 'string' &&
    (type.endsWith('.done') || type.endsWith('_stop') || type.endsWith('.completed'))
}

function sanitizeTraceContent(content: string, redact: (content: string) => string): string | undefined {
  const trimmed = redact(content.trim())
  if (!trimmed) return undefined
  return trimmed.length > 760 ? `${trimmed.slice(0, 760)}...` : trimmed
}

function dedupeProviderTraces(traces: ProcessTrace[]): ProcessTrace[] {
