...
export function extractOpenAIOutputText(output: unknown): string {
  ...
  for (const item of output) {
    ...
    if (isReasoningOutputType(itemType)) continue
    ...
    if (typeof content === 'string') {
      ...
      for (const part of content) {
        ...
        const contentType = stringValue(contentPart.type)
        if (isReasoningOutputType(contentType)) continue
...
function isReasoningOutputType(type: string): boolean {
