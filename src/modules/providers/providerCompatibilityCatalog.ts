import type { AIModel, AIProvider, ProviderPresetId, ProviderType, ProviderWireProtocol } from '@/types/providerContracts'
...
export type ProviderCompatibilityEvidenceId = ProviderPresetId | ProviderWireProtocol
...
export interface ProviderCompatibilityCapabilitySendPolicy extends ProviderCompatibilityCapabilityExplanation {
  ...
  compatibilityId: ProviderCompatibilityEvidenceId
...
function identityEvidence(input: ProviderCompatibilityEvidence): ProviderCompatibilityEvidence {
  return input
}
...
export const PROVIDER_COMPATIBILITY_EVIDENCE = {
  'custom-endpoint': identityEvidence({
    id: 'custom-endpoint',
    ...
    notes: 'Custom endpoint is a supplier identity only. Runtime compatibility evidence is resolved from its separately selected wire protocol.',
  }),
  openai: evidence({
    id: 'openai',
    ...
    notes: 'Primary Responses path with server-side context_management compaction; Chat Completions remains a compatibility path for older models and relays.',
  }),
  anthropic: evidence({
    id: 'anthropic',
    ...
    behaviorDocs: ['auth', 'chat', 'streaming', 'tools', 'structuredOutput', 'nativeSearch', 'vision', 'files', 'reasoning', 'modelList', 'errors', 'rateLimits', 'deprecation', 'citations', 'remoteCompact'],
    auditState: 'conformance-ready',
    notes: 'Native Messages request shape with thinking, tool-use, citation, web-search, and beta server-side compaction (compact-2026-01-12).',
  }),
  google: evidence({
    id: 'google',
    ...
    auditState: 'conformance-ready',
    notes: 'Native Gemini generateContent path with documented context caching. Caching reduces repeated-prefix processing but is not native context compaction; remoteCompact remains unclaimed. Vertex-hosted native routing is tracked separately under vertex-ai.',
  }),
  deepseek: evidence({
    id: 'deepseek',
    protocol: 'openai-compatible',
  ...
  xai: evidence({
    ...
    auditState: 'conformance-ready',
    notes: 'OpenAI-compatible surface with xAI Responses, response_format structured outputs, native web_search, encrypted reasoning replay, model metadata, and reasoning-effort differences. Current catalog evidence documents context-window behavior but not a native compaction request contract, so remote compaction remains unknown and unsendable.',
  }),
  'xiaomi-mimo': evidence({
    ...
    auditState: 'conformance-ready',
    notes: 'Current official sources document OpenAI-compatible Chat Completions and Codex configuration. Anthropic-compatible routing remains a user-selected compatibility mode with vendor support Unknown; Xiaomi Responses remains unclaimed and disabled.',
  }),
  mistral: evidence({
    id: 'mistral',
    protocol: 'openai-compatible',
  ...
  'github-models': evidence({
    ...
    endpointFamilies: ['/inference/chat/completions', '/catalog/models'],
    behaviorDocs: [...CORE_PROVIDER_COMPATIBILITY_BEHAVIORS, 'tools', 'vision', 'modelList', 'hostedRouting'],
    auditState: 'conformance-ready',
    notes: 'GitHub Models documents OpenAI-compatible inference with GitHub token auth and a separate GET /catalog/models discovery route. IsleMind preserves Bearer inference auth and adds only the documented GitHub JSON media type and conservative 2022-11-28 API version headers. Generic file_data attachments, audio, speech, reasoning_effort, embeddings, rerank, structured output request controls, and OpenAI Responses routing stay unclaimed until official docs and app routing prove those surfaces.',
  }),
  deepinfra: evidence({
    id: 'deepinfra',
    protocol: 'openai-compatible',
  ...
  'vertex-ai': evidence({
    ...
    behaviorStatusOverrides: { structuredOutput: 'requiresLiveKey' },
    liveSmoke: [
      liveSmokeGate({
  ...
  openrouter: evidence({
    ...
    officialDocs: [
      ...
      'https://openrouter.ai/docs/api_reference/responses/tool-calling',
    ...
    auditState: 'conformance-ready',
    notes: 'Aggregator route preserves upstream model capability differences. Offline fixtures cover chat routing, explicit Responses opt-in, response_format structured outputs gated by model supported_parameters, PDF file_data input on Chat Completions, tool declarations, model sync metadata, JSON/SSE parsing, retrieval citations, and rate-limit diagnostics. OpenRouter message transforms and upstream routing are not evidence of a native compaction contract, so remote compaction remains unknown and unsendable.',
  }),
  newapi: evidence({
    id: 'newapi',
    protocol: 'openai-compatible',
  ...
  'openai-compatible': evidence({
    ...
    auditState: 'protocol-reference',
    notes: 'Custom OpenAI-compatible endpoints are protocol references until declared, probed, or manually configured capabilities are present. The preset keeps optional tools, structured output, image/file attachments, reasoning controls, native search, audio, and Responses routing off by default; users or remote model metadata can explicitly enable supported behavior. Native context compaction remains unsupported until direct provider evidence is added.',
  }),
  'anthropic-compatible': evidence({
    id: 'anthropic-compatible',
    protocol: 'anthropic-compatible',
    officialDocs: ['https://platform.claude.com/docs/en/api/messages/create'],
    endpointFamilies: ['/v1/messages', '/v1/models'],
    behaviorDocs: [...CORE_PROVIDER_COMPATIBILITY_BEHAVIORS, 'modelList'],
    liveSmoke: [
      liveSmokeGate({
        id: 'anthropic-compatible-messages',
        kind: 'custom-endpoint',
    ...
    auditState: 'protocol-reference',
    notes: 'Custom Anthropic-compatible endpoints are protocol references until declared, probed, or manually configured capabilities are present. The preset keeps optional image/file attachments and reasoning controls off by default; Anthropic message/tool request shapes can be used only when a compatible endpoint is selected or declared. Native Messages compaction remains unsupported until direct Anthropic evidence is established.',
  }),
} as const satisfies Record<ProviderCompatibilityEvidenceId, ProviderCompatibilityEvidence>

export function getProviderCompatibilityEvidence(id: ProviderCompatibilityEvidenceId): ProviderCompatibilityEvidence {
  return PROVIDER_COMPATIBILITY_EVIDENCE[id]
}

export function resolveProviderCompatibilityEvidenceId(provider: ProviderCompatibilityEvidenceProviderLike): ProviderCompatibilityEvidenceId {
  if (provider.presetId === 'custom-endpoint' || provider.detectedPresetId === 'custom-endpoint') {
  ...
  if (provider.type === 'anthropic') return provider.wireProtocol === 'anthropic-compatible' ? 'anthropic-compatible' : 'anthropic'
...
/**
 * Native context compaction is an identity-bound capability.  A compatible
 * wire protocol, a Responses-shaped endpoint, or a user declaration is not
 * enough to promote it: the current catalog only has direct evidence for the
 * OpenAI provider and the Anthropic provider.
 */
export function providerNativeRemoteCompactEvidenceMatchesProvider(
  provider: ProviderCompatibilityEvidenceProviderLike,
): boolean {
  const evidence = getProviderCompatibilityEvidenceForProvider(provider)
  if (evidence.id === 'openai') {
    return provider.type === 'openai' && provider.wireProtocol === undefined
  }
  if (evidence.id === 'anthropic') {
    return provider.type === 'anthropic' && provider.wireProtocol === undefined
  }
  return false
}
...
export function resolveProviderCompatibilityCapabilitySendPolicy(
  ...
  if (behavior === 'remoteCompact' && !providerNativeRemoteCompactEvidenceMatchesProvider(provider)) {
    return {
      behavior,
      ...
      sendSource: 'blocked',
      limitationReason: 'contract_unclaimed',
      degradationPath: evidence.auditState === 'protocol-reference'
        ? 'manual_declaration_or_model_metadata'
        : 'disable_parameter',
    }
  }
...
function providerCompatibilityEvidenceMatchesProvider(
  ...
  if (id === 'openai-compatible' || id === 'anthropic-compatible') {
    return provider.wireProtocol === id
  }
...
export function getProviderCompatibilityLiveSmokeGates(id: ProviderCompatibilityEvidenceId): readonly ProviderCompatibilityLiveSmokeGate[] {
...
export function findProviderCompatibilityLiveSmokeGaps(presetIds: readonly ProviderPresetId[]): ProviderPresetId[] {
  return presetIds.filter((id) => {
    if (id === 'custom-endpoint') return false
    const evidence = PROVIDER_COMPATIBILITY_EVIDENCE[id]
