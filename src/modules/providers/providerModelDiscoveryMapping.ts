...
import type { AIModel, ModelReasoningMode, ProviderPresetId, ProviderType } from '@/types/providerContracts'
import { getModelConfig, mergeModelConfig, sortModelConfigs } from '@/types/modelCatalog'
...
function normalizeRemoteModelDisplayName(rawModelId: string, normalizedModelId: string, model: OpenAIModelListItem | undefined, providerType: ProviderType): string | undefined {
  ...
  if (knownName && modelNameLooksLikeTechnicalId(canonical)) {
    if (knownName.includes('/') && remoteModelIdLooksLikeDisplayLabel(source)) return canonical
    return knownName
  }
