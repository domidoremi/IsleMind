import type { AIModel, AIProvider, ProviderPresetId } from '@/types/providerContracts'
...
} from './providerAwsBedrockRouting'
import {
...
import {
  ...
  isVertexAIProvider,
} from './providerHostedBoundaryPolicy'
import { isPerplexityProvider } from './providerIdentityPolicy'
import {
  explainProviderBuiltinToolBlockReason,
...
export type ProviderCapabilityMatrixHostedOperation = 'chat' | 'modelList' | 'remoteCompact' | 'tools'

export interface ProviderCapabilityMatrixHostedIssue {
  message: string
}

export interface ProviderCapabilityMatrixNativeToolSupport {
  supported: boolean
  reason: string
}

export interface ProviderCapabilityMatrixPolicyDependencies {
  getModelConfig(modelId: string, providerType?: AIProvider['type'], modelConfigs?: AIModel[]): AIModel
  getProviderPreset(presetId: ProviderPresetId | undefined): { id: ProviderPresetId }
  getHostedProviderSupportIssue(
    provider: AIProvider,
    operation: ProviderCapabilityMatrixHostedOperation,
  ): ProviderCapabilityMatrixHostedIssue | null
  ...
  resolveProviderNativeToolSupport(provider: AIProvider, modelConfig: AIModel): ProviderCapabilityMatrixNativeToolSupport
...
export interface ProviderCapabilityMatrixPolicy {
  buildProviderCapabilityMatrix(provider: AIProvider): ProviderCapabilityMatrix
  buildProviderModelCapabilityMatrix(provider: AIProvider, modelId: string): ProviderModelCapabilityMatrix
  getProviderModelCapabilityModelIds(provider: AIProvider): string[]
  ...
  summarizeProviderCapabilityMatrix(matrix: ProviderCapabilityMatrix): string
  ...
  buildProviderCoverageBuckets(providers: AIProvider[]): Record<ProviderHostingProfile, number>
  providerNeedsHostedCompatibilityWork(provider: AIProvider): boolean
  providerSuppressesGenericModelList(provider: AIProvider): boolean
}
...
export function createProviderCapabilityMatrixPolicy(
  dependencies: ProviderCapabilityMatrixPolicyDependencies,
): ProviderCapabilityMatrixPolicy {
function buildProviderCapabilityMatrix(provider: AIProvider): ProviderCapabilityMatrix {
  const preset = dependencies.getProviderPreset(provider.presetId ?? provider.detectedPresetId)
...
function buildProviderModelCapabilityMatrix(provider: AIProvider, modelId: string): ProviderModelCapabilityMatrix {
...
function getProviderModelCapabilityModelIds(provider: AIProvider): string[] {
  return providerKnownModelIds(provider)
}

function summarizeProviderModelCapabilityProvider(
  provider: AIProvider,
...
function getProviderModelCapabilityStatus(
  provider: AIProvider,
  modelId: string,
  capability: ProviderModelCapabilityKey,
): ProviderModelCapabilityStatus | undefined {
...
function providerModelCapabilityCanBeSent(
  provider: AIProvider,
  modelId: string,
  capability: ProviderModelCapabilityKey,
): boolean {
...
function inferHostingProfile(provider: AIProvider): ProviderHostingProfile {
  ...
  if (dependencies.isAwsBedrockHostedProvider(provider) || isVertexAIProvider(provider)) return 'cloud-hosted'
  if (presetId === 'custom-endpoint') return 'relay'
...
function buildResponseStatus(provider: AIProvider, hostingProfile: ProviderHostingProfile): ProviderCapabilityStatus {
  ...
  const hostedIssue = hostingProfile === 'cloud-hosted' ? dependencies.getHostedProviderSupportIssue(provider, 'chat') : null
  if (hostedIssue) {
...
function buildStreamingStatus(provider: AIProvider): ProviderCapabilityStatus {
  ...
  const nonStreamingModels = providerKnownModelIds(provider)
    .filter((modelId) => dependencies.getModelConfig(modelId, provider.type, provider.modelConfigs).supportsStreaming === false)
...
function buildModelToolsCapability(provider: AIProvider, modelConfig: AIModel): ProviderModelCapabilityStatus {
  const decision = dependencies.resolveProviderNativeToolSupport(provider, modelConfig)
...
function buildModelVisionCapability(provider: AIProvider, modelConfig: AIModel): ProviderModelCapabilityStatus {
  if (!dependencies.providerSupportsVisionInput(provider, modelConfig)) {
...
function buildModelFilesCapability(provider: AIProvider, modelConfig: AIModel): ProviderModelCapabilityStatus {
  if (!dependencies.providerSupportsFileInput(provider, modelConfig)) {
...
function buildModelReasoningCapability(provider: AIProvider, modelConfig: AIModel): ProviderModelCapabilityStatus {
  if (!dependencies.providerSupportsReasoning(provider, modelConfig.id)) {
...
function buildModelNativeSearchCapability(provider: AIProvider, modelConfig: AIModel): ProviderModelCapabilityStatus {
  ...
  const openAIResponsesSearchPolicy = nativeSearchWouldUseOpenAIResponsesBuiltinTool(provider, modelConfig)
    ? resolveOpenAIResponsesWebSearchToolPolicy(provider, modelConfig.id)
    : undefined
  if (openAIResponsesSearchPolicy && !openAIResponsesSearchPolicy.allowed && openAIResponsesSearchPolicy.reason) {
    return modelCapability('nativeSearch', 'unsupported', 'unsupported-contract', explainProviderBuiltinToolBlockReason(openAIResponsesSearchPolicy.reason))
  }
...
function providerModelWasTested(provider: AIProvider, modelId: string): boolean {
  const testedModel = provider.lastTestModel ? dependencies.resolveProviderModelAlias(provider, provider.lastTestModel) : undefined
...
function nativeSearchWouldUseOpenAIResponsesBuiltinTool(provider: AIProvider, modelConfig: AIModel): boolean {
  if (provider.type !== 'openai-compatible') return false
  if (provider.wireProtocol === 'anthropic-compatible') return false
  return provider.capabilities?.responsesApi === true || modelConfig.preferredEndpoint === 'responses'
}
...
function buildModelCatalogStatus(provider: AIProvider, hostingProfile: ProviderHostingProfile): ProviderCapabilityStatus {
  ...
  const hostedIssue = hostingProfile === 'cloud-hosted' ? dependencies.getHostedProviderSupportIssue(provider, 'modelList') : null
  if (hostedIssue) {
...
function buildDeprecationStatus(provider: AIProvider): ProviderCapabilityStatus {
  ...
  const deprecatedModels = providerKnownModelIds(provider)
    .map((modelId) => ({ modelId, config: dependencies.getModelConfig(modelId, provider.type, provider.modelConfigs) }))
...
function providerNativeCitationParserCovered(evidenceId: ProviderCompatibilityEvidenceId): boolean {
...
function buildVisionStatus(provider: AIProvider): ProviderCapabilityStatus {
  ...
  const visionModels = modelIds
    .map((modelId) => ({ modelId, config: dependencies.getModelConfig(modelId, provider.type, provider.modelConfigs) }))
    .filter((item) => dependencies.providerSupportsVisionInput(provider, item.config))
...
function buildFileStatus(provider: AIProvider): ProviderCapabilityStatus {
  ...
  const fileModels = modelIds
    .map((modelId) => ({ modelId, config: dependencies.getModelConfig(modelId, provider.type, provider.modelConfigs) }))
    .filter((item) => dependencies.providerSupportsFileInput(provider, item.config))
...
function buildReasoningStatus(provider: AIProvider): ProviderCapabilityStatus {
  ...
  const reasoningModels = providerKnownModelIds(provider)
    .map((modelId) => ({ modelId, config: dependencies.getModelConfig(modelId, provider.type, provider.modelConfigs) }))
    .filter((item) => dependencies.providerSupportsReasoning(provider, item.modelId))
...
function buildRemoteCompactStatus(provider: AIProvider, _hostingProfile: ProviderHostingProfile): ProviderCapabilityStatus {
  ...
  if (directOpenAI && provider.capabilities?.remoteCompact === true && provider.capabilities?.responsesApi === true && contractStatus === 'supported' && providerCompatibilityCapabilityCanBeSentForProvider(provider, 'remoteCompact', true)) {
    return { area: 'remoteCompact', level: 'full', contractStatus, reason: 'direct OpenAI Responses declares contract-backed native context_management compaction' }
  }
  if (directAnthropic && provider.capabilities?.remoteCompact === true && contractStatus === 'supported' && providerCompatibilityCapabilityCanBeSentForProvider(provider, 'remoteCompact', true)) {
    return { area: 'remoteCompact', level: 'full', contractStatus, reason: 'direct Anthropic Messages declares beta native compaction (compact-2026-01-12 / compact_20260112)' }
  }
  if (evidenceId === 'google' || provider.type === 'google') {
    return { area: 'remoteCompact', level: 'unsupported', contractStatus, reason: 'Gemini context caching is evidenced, but it is caching rather than native context compaction' }
  }
  if (evidenceId === 'xai' || evidenceId === 'openrouter') {
    return { area: 'remoteCompact', level: 'partial', contractStatus, reason: 'context-window behavior or upstream message transforms are known, but native compaction remains unknown and no compact field is sent' }
  }
  return { area: 'remoteCompact', level: 'unsupported', contractStatus, reason: 'generic compatible or provider-specific endpoint has no evidenced native compaction contract' }
}
...
function buildToolStatus(provider: AIProvider, hostingProfile: ProviderHostingProfile): ProviderCapabilityStatus {
  ...
  const hostedIssue = hostingProfile === 'cloud-hosted' ? dependencies.getHostedProviderSupportIssue(provider, 'tools') : null
  if (hostedIssue) {
...
function summarizeProviderCapabilityMatrix(matrix: ProviderCapabilityMatrix): string {
  return `${matrix.hostingProfile} · ${matrix.summaryLevel}`
}

function summarizeProviderCapabilityMatrixDetails(
  provider: AIProvider,
...
function describeProviderCapabilityStatus(matrix: ProviderCapabilityMatrix, area: ProviderCapabilityArea): string | undefined {
  return matrix.statuses.find((status) => status.area === area)?.reason
}

function buildProviderCoverageBuckets(providers: AIProvider[]): Record<ProviderHostingProfile, number> {
...
function providerNeedsHostedCompatibilityWork(provider: AIProvider): boolean {
  return dependencies.isHostedProviderGap(provider)
}

function providerSuppressesGenericModelList(provider: AIProvider): boolean {
