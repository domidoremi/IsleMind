...
export type ProviderHealthFailureTrigger =
  | 'timeout'
  | 'network_error'
  | 'rate_limited'
  | 'server_error'
...
export interface ProviderHealthCandidate extends ProviderHealthRouteKey {
  model: string
  healthy?: boolean
...
export interface ProviderHealthPolicy {
  ...
  triggerCooldownMs?: Partial<Record<ProviderHealthFailureTrigger, number>>
}
...
const DEFAULT_TRIGGER_COOLDOWN_MS: Partial<Record<ProviderHealthFailureTrigger, number>> = {
  timeout: 10_000,
...
export function annotateFailoverCandidatesWithHealth<TCandidate extends ProviderHealthCandidate>(
...
function cooldownMsForTrigger(trigger: ProviderHealthFailureTrigger, policy: ProviderHealthPolicy): number {
