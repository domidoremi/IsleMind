...
import type { ProxyMode, Settings } from '@/types/settingsContracts'
import type { ProviderFailoverDecision, ProviderFailoverRoute } from './providerFailoverPolicy'
import type { ProviderHealthStatus } from './providerHealth'
...
export interface ProviderProxyPolicyDependencies {
  safeHttpUrl(value: string | undefined): string | null
}

export interface ProviderProxyPolicy {
  resolveProxyPolicy(input: ProxyPolicyInput): ProxyPolicyDecision
}

export function createProviderProxyPolicy(
  dependencies: ProviderProxyPolicyDependencies,
): ProviderProxyPolicy {
  function resolveProxyPolicy(input: ProxyPolicyInput): ProxyPolicyDecision {
    ...
    const baseUrl = dependencies.safeHttpUrl(input.settings?.proxyBaseUrl)
