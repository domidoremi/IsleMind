...
export const PROVIDER_REQUEST_SHAPING_COMPATIBILITY_FIXTURES: ProviderRequestShapingFixture[] = [
  ...
  {
    id: 'visible-downgrade-compatible-builtin-search-overclaim',
    requestShape: 'openai-compatible',
    hostingProfile: 'aggregator',
    expectedReadiness: 'degraded',
    description: 'Gemini, Qwen3-Coder, LongCat, MiMo-compatible, and MiniMax routes can expose optimistic search metadata without accepting OpenAI Responses built-in web_search tools.',
    policy: {
      ...SAFE_REQUEST_SHAPING_POLICY,
      requiredCapabilities: ['text', 'native-search', 'token-budget'],
      supportedCapabilities: ['text', 'token-budget'],
...
export function evaluateProviderRequestShapingCompatibilityQualityGate(
  ...
  requireDegraded(byId.get('visible-downgrade-compatible-builtin-search-overclaim'), failures, 'visible-downgrade-compatible-builtin-search-overclaim', { requireFallback: true, requireRemovedField: 'web_search_preview' })
