import type { PayloadPolicyMode } from '@/types/settingsContracts'

export type PayloadRuleId =
  | 'empty_messages'
  | 'oversized_body'
  | 'attachment_base64'
  | 'too_many_messages'

export interface ProviderPayloadRuleInput {
  body: unknown
...
export interface ProviderPayloadRuleFinding {
  id: PayloadRuleId
  severity: 'warn' | 'block'
  message: string
  value?: number
...
export interface ProviderPayloadRuleResult {
  mode: PayloadPolicyMode
  blocked: boolean
  findings: ProviderPayloadRuleFinding[]
  bodyKeys: string[]
  messageCount: number
  attachmentCount: number
}

const DEFAULT_MAX_BODY_BYTES = 1_800_000
const DEFAULT_MAX_MESSAGES = 256

export function evaluateProviderPayloadRules(
  input: ProviderPayloadRuleInput,
): ProviderPayloadRuleResult {
  ...
  const findings: ProviderPayloadRuleFinding[] = []
  ...
  if (messageCount > maxMessages) {
    findings.push(finding(
      'too_many_messages',
      mode,
      'payload exceeds message count limit',
      messageCount,
      maxMessages,
    ))
  }
  ...
  if (bodyBytes > maxBodyBytes) {
    findings.push(finding(
      'oversized_body',
      mode,
      'payload exceeds body size limit',
      bodyBytes,
      maxBodyBytes,
    ))
  }
  ...
  if (attachmentCount && hasEmbeddedAttachmentData(input.body)) {
    findings.push(finding(
      'attachment_base64',
      mode,
      'payload contains inline attachment data',
      attachmentCount,
    ))
  }
