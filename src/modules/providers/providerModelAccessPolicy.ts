import type { AIProvider, ModelAlias } from '@/types/providerContracts'
...
export interface ProviderModelAccessSettings {
...
export interface ProviderModelAccessPolicyDependencies {
  getProviderManualModels(provider: AIProvider): string[]
  getProviderPreferredModel(provider: AIProvider): string | undefined
  inferModelFamily(provider: AIProvider | undefined, model: string | undefined): string
  isProviderChatCompatibleModel(provider: AIProvider, model: string): boolean
  normalizeProviderModelAliases(provider: Pick<AIProvider, 'modelAliases'>): ModelAlias[]
  resolveProviderModelAlias(provider: Pick<AIProvider, 'modelAliases'>, model: string): string
}

export interface ProviderModelAccessPolicy {
  hasProviderModelAccessRules(settings?: ProviderModelAccessSettings): boolean
  resolveProviderModelAccess(input: ProviderModelAccessInput): AccessPolicyDecision
  resolveProviderModelAliasAccess(input: ProviderModelAliasAccessInput): AccessPolicyDecision
  mergeRuntimeAliasAccessPolicy(
    requested: AccessPolicyDecision,
    upstream: AccessPolicyDecision,
  ...
  getPolicyPreferredProviderModel(provider: AIProvider, settings?: ProviderModelAccessSettings): string | undefined
  providerHasPolicyAllowedModel(provider: AIProvider, settings?: ProviderModelAccessSettings): boolean
  providerHasPolicyModel(provider: AIProvider, model: string, settings?: ProviderModelAccessSettings): boolean
  getProviderModelDisplayCandidates(input: ProviderModelDisplayPolicyInput): ProviderModelDisplayCandidate[]
}
...
export function createProviderModelAccessPolicy(
  dependencies: ProviderModelAccessPolicyDependencies,
): ProviderModelAccessPolicy {
function hasProviderModelAccessRules(settings?: ProviderModelAccessInput['settings']): boolean {
  return Boolean(
...
function resolveProviderModelAccess(input: ProviderModelAccessInput): AccessPolicyDecision {
...
function resolveProviderModelAliasAccess(input: ProviderModelAliasAccessInput): AccessPolicyDecision {
  const upstreamModel = dependencies.resolveProviderModelAlias(input.provider, input.model)
...
function mergeRuntimeAliasAccessPolicy(requested: AccessPolicyDecision, upstream: AccessPolicyDecision): AccessPolicyDecision {
...
function getPolicyAllowedProviderModels(provider: AIProvider, settings?: ProviderModelAccessInput['settings'], options: { limit?: number } = {}): string[] {
  ...
  const pushAllowed = (model: string | undefined): boolean => {
    ...
    seen.add(normalized)
    if (!dependencies.isProviderChatCompatibleModel(provider, normalized)) return false
  ...
  const manualModels = dependencies.getProviderManualModels(provider)
  ...
  for (const alias of dependencies.normalizeProviderModelAliases(provider)) {
...
function getPolicyPreferredProviderModel(provider: AIProvider, settings?: ProviderModelAccessInput['settings']): string | undefined {
  const preferred = dependencies.getProviderPreferredModel(provider)
  if (
    preferred &&
    dependencies.isProviderChatCompatibleModel(provider, preferred) &&
    providerHasAvailableSourceModel(provider, preferred) &&
    (!hasProviderModelAccessRules(settings) || resolveProviderModelAliasAccess({ provider, model: preferred, settings }).allowed)
  ) return preferred
...
function providerHasPolicyModel(provider: AIProvider, model: string, settings?: ProviderModelAccessInput['settings']): boolean {
  const normalized = model.trim()
  return !!normalized &&
    dependencies.isProviderChatCompatibleModel(provider, normalized) &&
...
function getProviderModelDisplayCandidates(input: ProviderModelDisplayPolicyInput): ProviderModelDisplayCandidate[] {
...
function matchModelScope(provider: ProviderModelAccessProvider, model: string, list: string[] | undefined): string | null {
  const family = dependencies.inferModelFamily(provider as AIProvider, model)
  return matchScopedValues([
    model,
...
function providerHasAvailableSourceModel(provider: AIProvider, targetModel: string): boolean {
  ...
  return dependencies.normalizeProviderModelAliases(provider).some((alias) =>
