...
} from './providerNativeSearch'
import type {
  ProviderRequestHardeningInput,
  ProviderRequestHardeningResult,
} from './providerRequestHardeningPolicy'
import type {
  ProviderRequestParameterPlan,
...
} from './providerToolCapabilityPolicy'
...
export interface ProviderConformancePolicyDependencies {
  getModelConfig(model: string, providerType?: ProviderType, modelConfigs?: AIModel[]): AIModel
  filterSendableAttachments(attachments: Attachment[] | undefined): Attachment[]
  hardenProviderRequestBody(input: ProviderRequestHardeningInput): ProviderRequestHardeningResult
  getReasoningEffortOptions(provider: AIProvider | undefined, model: string | undefined): ReasoningEffort[]
  isCerebrasReasoningModel(provider: AIProvider, model: string): boolean
  isClaudeThinkingModel(provider: AIProvider, model: string): boolean
  isCohereReasoningModel(provider: AIProvider, model: string): boolean
  isDashScopeThinkingModel(provider: AIProvider, model: string): boolean
  isDeepInfraReasoningModel(provider: AIProvider, model: string): boolean
  isDeepSeekThinkingModel(provider: AIProvider, model: string): boolean
  isFireworksReasoningModel(provider: AIProvider, model: string): boolean
  isGeminiThinkingLevelModel(model: string): boolean
  isGeminiThinkingModel(provider: AIProvider, model: string): boolean
  isGroqReasoningModel(provider: AIProvider, model: string): boolean
  isHuggingFaceReasoningModel(provider: AIProvider, model: string): boolean
  ...
  isOpenAIReasoningModel(provider: AIProvider, model: string): boolean
  isPerplexityReasoningModel(provider: AIProvider, model: string): boolean
  isSambaNovaReasoningModel(provider: AIProvider, model: string): boolean
  isSiliconFlowReasoningModel(provider: AIProvider, model: string): boolean
  isTogetherReasoningModel(provider: AIProvider, model: string): boolean
  isXAIReasoningModel(provider: AIProvider, model: string): boolean
  isXAIMultiAgentReasoningModel(model: string): boolean
...
export interface ProviderConformancePolicy {
  resolveProviderCapabilityManifest(input: ProviderConformanceRequest): ProviderCapabilityManifest
...
export const PROVIDER_GENERATION_PARAMETER_DIAGNOSTICS_SCHEMA = 'islemind.provider-generation-parameter-diagnostics.v1'
...
export interface ProviderCapabilityManifest {
  ...
  tools: {
    ...
    nativeWebSearchToolType?: 'web_search_preview' | 'web_search' | 'web_search_20250305' | 'web_search_20260209' | 'web_search_20260318'
  }
  structuredOutput: {
...
export type ProviderGenerationParameterFinalAction =
  | 'omitted-as-planned'
  | 'retained'
...
export interface ProviderGenerationParameterDiagnosticEntry extends ProviderRequestParameterPlanEntry {
  finalDecision: 'included' | 'omitted'
  finalAction: ProviderGenerationParameterFinalAction
  effectiveValue?: number
}

export interface ProviderGenerationParameterDiagnostics {
  schema: typeof PROVIDER_GENERATION_PARAMETER_DIAGNOSTICS_SCHEMA
  endpoint: ProviderRequestParameterPlan['endpoint']
  entries: Readonly<Record<GenerationParameterKey, ProviderGenerationParameterDiagnosticEntry>>
}
...
export function createProviderConformancePolicy(
  dependencies: ProviderConformancePolicyDependencies,
): ProviderConformancePolicy {
  function resolveProviderCapabilityManifest(input: ProviderConformanceRequest): ProviderCapabilityManifest {
  const provider = input.provider
  const modelConfig = dependencies.getModelConfig(input.model, provider.type, provider.modelConfigs)
  ...
  const selectableEfforts = dependencies.getReasoningEffortOptions(provider, input.model)
  ...
  const toolCapabilities = PROVIDER_TOOL_CAPABILITY_POLICY.resolveProviderToolCapabilityManifest(provider, modelConfig)
  ...
  return {
    ...
    modalities: {
      input: {
        text: true,
        image: toolCapabilities.modalities.input.image,
        file: toolCapabilities.modalities.input.file,
        audio: toolCapabilities.modalities.input.audio,
        video: family === 'minimax' && isMiniMaxM3Model(input.provider, input.model),
      },
      output: {
        text: true,
        speech: toolCapabilities.modalities.output.speech,
      },
    },
    reasoning: {
    ...
    tools: {
      supported: toolCapabilities.tools.supported,
...
function resolveAndHardenProviderRequest(input: ProviderConformanceRequest, body: Record<string, unknown>): ProviderBodyConformanceResult {
  const manifest = resolveProviderCapabilityManifest(input)
  const reasoningIssues: ProviderConformanceIssue[] = []
  const reasoning = resolveReasoning(input, manifest, reasoningIssues)
  const requestModalities = requestedModalities(input.attachments)
  const hardened = dependencies.hardenProviderRequestBody({
    body,
    manifest,
    reasoning,
    requestedModalities: requestModalities,
    structuredOutput: input.structuredOutput,
  })
  const issues: ProviderConformanceIssue[] = [...reasoningIssues, ...hardened.issues]
  const reasoningResolution = buildProviderReasoningResolutionArtifact(
    manifest,
    reasoning,
    issues,
    hardened.removedParams,
  )
  const parameterDiagnostics = input.generationParameterPlan
    ? buildGenerationParameterDiagnostics(input.generationParameterPlan, hardened.body)
    : undefined
  ...
  return {
    ...
    conformance: {
      ...
      requestedModalities: requestModalities,
      issues,
      removedParams: hardened.removedParams,
      adjustedParams: hardened.adjustedParams,
      bodyKeys: Object.keys(hardened.body).sort(),
      ...(parameterDiagnostics ? { parameterDiagnostics } : {}),
    },
  }
}

function buildGenerationParameterDiagnostics(
  plan: ProviderRequestParameterPlan,
  body: Record<string, unknown>,
): ProviderGenerationParameterDiagnostics {
  const entries = {} as Record<GenerationParameterKey, ProviderGenerationParameterDiagnosticEntry>
  for (const parameter of ['temperature', 'topP', 'topK', 'maxTokens'] as const) {
    const planned = plan.entries[parameter]
    const effectiveValue = planned.wirePath ? readNumericWireValue(body, planned.wirePath) : undefined
    if (planned.decision === 'omitted') {
      entries[parameter] = effectiveValue === undefined
        ? { ...planned, finalDecision: 'omitted', finalAction: 'omitted-as-planned' }
        : { ...planned, finalDecision: 'included', finalAction: 'unexpectedly-included', effectiveValue }
      continue
    }
    if (effectiveValue === undefined) {
      entries[parameter] = { ...planned, finalDecision: 'omitted', finalAction: 'removed-after-serialization' }
      continue
    }
    entries[parameter] = planned.value === effectiveValue
      ? { ...planned, finalDecision: 'included', finalAction: 'retained', effectiveValue }
      : { ...planned, finalDecision: 'included', finalAction: 'adjusted-after-serialization', effectiveValue }
  }
  return {
    schema: PROVIDER_GENERATION_PARAMETER_DIAGNOSTICS_SCHEMA,
    endpoint: plan.endpoint,
    entries,
  }
}

function readNumericWireValue(body: Record<string, unknown>, path: readonly string[]): number | undefined {
...
function applyProviderConformanceToBody(input: ProviderConformanceRequest, body: Record<string, unknown>): Record<string, unknown> {
  return resolveAndHardenProviderRequest(input, body).body
}

function resolveProviderRequestConformance(input: ProviderConformanceRequest, body: Record<string, unknown>): ProviderConformanceResult {
...
function isMiniMaxM3Model(provider: AIProvider, model: string): boolean {
  return dependencies.isMiniMaxThinkingModel(provider, model)
}
...
function inferReasoningRequestShape(input: ProviderConformanceRequest, family: ProviderConformanceFamily, protocol: ProviderConformanceProtocol): ProviderReasoningRequestShape {
  ...
  if (providerProtocolReferenceDisablesCapability(input.provider, config, 'reasoningEffort')) return 'none'
  if (family === 'openai' && dependencies.isOpenAIReasoningModel(input.provider, input.model)) {
  ...
  if (family === 'anthropic' || family === 'anthropic-compatible') {
    if (config.reasoningMode === 'anthropic-thinking' || dependencies.isClaudeThinkingModel(input.provider, input.model)) {
  ...
  if (family === 'google' && dependencies.isGeminiThinkingModel(input.provider, input.model)) {
    return dependencies.isGeminiThinkingLevelModel(input.model) || config.reasoningMode === 'gemini-thinking-level' ? 'gemini-thinking-level' : 'gemini-thinking-budget'
  }
  ...
  if (family === 'groq' && (config.reasoningMode === 'groq-reasoning-effort' || dependencies.isGroqReasoningModel(input.provider, input.model))) return 'groq-reasoning-effort'
  if (family === 'together' && (config.reasoningMode === 'together-reasoning-effort' || dependencies.isTogetherReasoningModel(input.provider, input.model))) return 'together-reasoning-effort'
  if (family === 'fireworks' && (config.reasoningMode === 'fireworks-reasoning-effort' || dependencies.isFireworksReasoningModel(input.provider, input.model))) return 'fireworks-reasoning-effort'
  if (family === 'perplexity' && (config.reasoningMode === 'perplexity-reasoning-effort' || dependencies.isPerplexityReasoningModel(input.provider, input.model))) return 'perplexity-reasoning-effort'
  if (family === 'cohere' && (config.reasoningMode === 'cohere-reasoning-effort' || dependencies.isCohereReasoningModel(input.provider, input.model))) return 'cohere-reasoning-effort'
  if (family === 'cerebras' && (config.reasoningMode === 'cerebras-reasoning-effort' || dependencies.isCerebrasReasoningModel(input.provider, input.model))) return 'cerebras-reasoning-effort'
  if (family === 'sambanova' && (config.reasoningMode === 'sambanova-reasoning-effort' || dependencies.isSambaNovaReasoningModel(input.provider, input.model))) return 'sambanova-reasoning-effort'
  if (family === 'huggingface' && (config.reasoningMode === 'huggingface-reasoning-effort' || dependencies.isHuggingFaceReasoningModel(input.provider, input.model))) return 'huggingface-reasoning-effort'
  if (family === 'deepinfra' && (config.reasoningMode === 'deepinfra-reasoning-effort' || dependencies.isDeepInfraReasoningModel(input.provider, input.model))) return 'deepinfra-reasoning-effort'
  if (family === 'siliconflow' && (config.reasoningMode === 'siliconflow-thinking-budget' || dependencies.isSiliconFlowReasoningModel(input.provider, input.model))) return 'siliconflow-thinking-budget'
  if (family === 'xiaomi-mimo' && dependencies.isXiaomiMimoReasoningModel(input.provider, input.model)) return 'xiaomi-mimo-thinking'
  return 'none'
}
...
function inferStructuredOutput(
  ...
  const modelConfig = dependencies.getModelConfig(input.model, input.provider.type, input.provider.modelConfigs)
...
function modelSupportsOpenAIResponseFormat(modelConfig: AIModel, protocol: ProviderConformanceProtocol): boolean {
...
function nativeWebSearchToolType(
  ...
  const modelConfig = dependencies.getModelConfig(input.model, input.provider.type, input.provider.modelConfigs)
  if (providerProtocolReferenceDisablesCapability(input.provider, modelConfig, 'nativeSearch')) return undefined
  if (family === 'openai' && protocol === 'openai-responses') return 'web_search'
  ...
  if (family === 'anthropic' || family === 'anthropic-compatible') {
    if (!supportsAnthropicDynamicWebSearch(input.model)) return 'web_search_20250305'
    return family === 'anthropic' ? 'web_search_20260318' : 'web_search_20260209'
  }
  return undefined
}
...
function providerReasoningValue(shape: ProviderReasoningRequestShape, effort: ReasoningEffort | undefined, model: string): string | number | boolean | undefined {
  ...
  if (shape === 'xai-reasoning-effort') {
    if (dependencies.isXAIMultiAgentReasoningModel(model)) {
  ...
  if (shape === 'fireworks-reasoning-effort') return dependencies.normalizeFireworksReasoningEffort(model, effort)
...
function requestedModalities(attachments: Attachment[] | undefined): string[] {
  ...
  for (const attachment of dependencies.filterSendableAttachments(attachments)) {
...
function providerSourceUrl(family: ProviderConformanceFamily): string | undefined {
  switch (family) {
    ...
    case 'xiaomi-mimo':
      return 'https://mimo.mi.com/docs/en-US/quick-start/model'
    default:
      return undefined
  }
}
