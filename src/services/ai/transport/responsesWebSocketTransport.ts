...
export interface ResponsesWebSocketTransportInput {
  ...
  req: ChatRequest
  signal: AbortSignal
  parseEvent: (payload: unknown, providerType: ProviderType) => { text: string; traces: ProcessTrace[]; usage?: MessageUsage; responseId?: string }
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (payload?.type === 'response.completed' || payload?.type === 'response.done') {
          ...
          input.onDone({ text: fullText, citations, traces, usage, responseId } satisfies ChatCompletionResult)
          settle()
        }
      } catch (error) {
