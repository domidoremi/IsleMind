...
describe('background environment', () => {
  ...
  test('motion and intensity stay conservative while respecting user choice', () => {
    ...
    expect(immersive.layerCount).toBe(3)
    expect(low.layerCount).toBe(3)
    expect(immersive.cycleMs).toBeGreaterThanOrEqual(18_000)
    expect(low.cycleMs).toBeGreaterThan(immersive.cycleMs)
    expect(low.grainOpacity).toBe(0)
