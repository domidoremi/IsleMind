...
import { SETTINGS_DESTINATIONS } from '@/presentation/features/settings/settingsRegistry'
import { SettingsPageShell } from './SettingsPageShell'
import { RadioGroup } from 'animal-island-ui-rn'
import { memo, useCallback, useEffect, useMemo, useRef, useState, type ReactNode } from 'react'
import { StyleSheet, Text, View, useWindowDimensions, type GestureResponderEvent, type ViewStyle } from 'react-native'
...
import { type SettingsSummaryItem } from '@/components/settings/SettingsSummaryStrip'
...
import { type SettingsControlSearchDocument } from '@/presentation/features/settings/settingsControlSearch'
...
import { SettingsThemeAccentControl, type ThemeAccentDraft } from '@/components/settings/SettingsThemeAccentControl'
import { ThemeRadioMark } from '@/components/settings/ThemeRadioMark'
...
const THEME_FAMILY_OPTIONS: { id: CanonicalThemeId; labelKey: string; detailKey: string }[] = [
  ...
  { id: 'animal-island-ui', labelKey: 'settings.themeAnimalIsland', detailKey: 'settings.themeAnimalIslandDescription' },
]
...
type SettingsChildRoute = RuntimeRepairSettingsRoute | '/settings/usage' | '/settings/preferences' | '/settings/memory' | '/settings/knowledge' | '/settings/agents'
...
export function SystemSettingsPanelContent({ panel }: { panel: SettingsControlPanel }) {
  const { t } = useTranslation()
  return <SettingsPageShell title={t('settingsWorkspace.' + ({ appearance: 'appearance', data: 'backup', advanced: 'notifications', diagnostics: 'diagnostics', governance: 'governance', updates: 'updates', danger: 'danger' }[panel]))}><SystemSettingsPanelBody panel={panel} /></SettingsPageShell>
}
const SystemSettingsPanelBody = memo(function SystemSettingsPanelBody({ panel }: { panel: SettingsControlPanel }) {
  const fieldSession = useSettingsFieldSession()
  const target = useSettingsTarget()
  const shellNavigation = false
  const { colors, canonicalThemeId } = useAppTheme()
  const sectionMotion = useThemeMotion('section', { readable: true, direction: 'forward' })
  const motion = sectionMotion.intensity
  const { t } = useTranslation()
  const dialog = useIsleDialog()
  const { width, fontScale } = useWindowDimensions()
  const narrowLayout = width / fontScale < 430
  const actionCompact = width / fontScale < 360
  ...
  const selectTheme = useThemeSelection(useCallback((error: unknown) => {
    dialog.toast({ title: t('settings.themeChangeFailed'), message: userFacingErrorDetail(error), tone: 'danger' })
  }, [dialog, t]), updateSettings)
  ...
  const themeAccentDraft = useRef<ThemeAccentDraft>({})
  const [savingObservabilitySinkApiKey, setSavingObservabilitySinkApiKey] = useState(false)
  const [observabilityKeyLoaded, setObservabilityKeyLoaded] = useState(false)
  const observabilityKeyBaseline = useRef('')
  const observabilitySaveLock = useRef(false)
  useSettingsDraft(observabilityKeyLoaded && observabilitySinkApiKeyDraft !== observabilityKeyBaseline.current, savingObservabilitySinkApiKey, () => setObservabilitySinkApiKeyDraft(observabilityKeyBaseline.current))
  ...
  const expandedGroups: Record<SettingsAdvancedGroup, boolean> = { ...COLLAPSED_SYSTEM_PANELS, [panel]: true, advanced: panel !== 'appearance' && panel !== 'data' }
  ...
  useEffect(() => {
    const group = SETTINGS_DESTINATIONS.find(entry => entry.section === target)?.parentSection as SettingsGovernanceGroup | undefined
    if (group) setExpandedGovernanceGroups(current => ({ ...current, [group]: true }))
  }, [target])
  ...
  const foldoutMotion = sectionMotion.transition
  ...
  const googlePlayDistribution = isGooglePlayDistribution()
  ...
  }, [panel, settings.systemStatusNotificationsEnabled])
  ...
  useEffect(() => {
    if (!expandedGroups.governance || !expandedGovernanceGroups.observability || observabilityKeyLoaded) return
    let cancelled = false
    void getObservabilitySinkApiKey().then((apiKey) => {
      if (!cancelled) { observabilityKeyBaseline.current = apiKey ?? ''; setObservabilitySinkApiKeyDraft(apiKey ?? ''); setObservabilityKeyLoaded(true) }
    })
    return () => {
      cancelled = true
    }
  ...
  async function saveObservabilitySinkApiKey() {
    if (!observabilityKeyLoaded || observabilitySaveLock.current) return
    observabilitySaveLock.current = true
    setSavingObservabilitySinkApiKey(true)
    try {
      const current = await getObservabilitySinkApiKey()
      if ((current ?? '') !== observabilityKeyBaseline.current) { dialog.toast({ title: t('settingsWorkspace.conflict'), tone: 'amber' }); return }
      await setObservabilitySinkApiKey(observabilitySinkApiKeyDraft)
      observabilityKeyBaseline.current = observabilitySinkApiKeyDraft.trim()
  ...
  function openRuntimeRepairTask(task: RuntimeRepairTask) {
    switch (task.target.kind) {
      ...
      case 'compact-settings':
        router.push({ pathname: '/settings/system/governance', params: { section: task.target.kind === 'session-affinity-settings' ? 'governance.sessionAffinityTtlMs' : 'governance.remoteCompactMode' } } as never)
        dialog.toast({
  ...
  return (
    <>
      ...
      {panel === 'appearance' && canUndo ? <IsleButton label={t('settingsWorkspace.undo')} onPress={undo} /> : null}
      <AnimatePresence>
      {controlView === 'system' && expandedGroups.appearance ? (
        <MotiView
          key="appearance-foldout"
          testID="settings-appearance-foldout"
          from={target ? undefined : sectionMotion.from}
        ...
        >
          ...
          />
          <RadioGroup testID="settings-theme-family-group" accessibilityLabel={t('settings.themeFamily')} style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            {THEME_FAMILY_OPTIONS.map((item) => (
              <ThemeFamilyCard
                ...
                onPress={(event) => selectTheme({ themeId: item.id }, event)}
          ...
          />
          <RadioGroup testID="settings-theme-mode-group" accessibilityLabel={t('settings.themeMode')} style={{ flexDirection: actionCompact ? 'column' : 'row', gap: 8 }}>
            {(['light', 'dark', 'system'] satisfies ThemeMode[]).map((item) => (
              <ThemeModeCard
                ...
                onPress={(event) => selectTheme({ theme: item }, event)}
          ...
          />
          <SettingsThemeAccentControl
            value={settings.themeAccent}
            draftRef={themeAccentDraft}
            onChange={(themeAccent, event) => selectTheme({ themeAccent }, event)}
          />
          <SettingsInlineLabel
            title={t('settings.backgroundEnvironment')}
            detail={t('settings.backgroundEnvironmentDetail')}
          />
          <RadioGroup testID="settings-background-variation-group" accessibilityLabel={t('settings.backgroundVariation')} style={{ gap: 6 }}>
            <Text style={{ color: colors.textSecondary, fontSize: 14, lineHeight: 20, fontWeight: '700' }}>{t('settings.backgroundVariation')}</Text>
          ...
          {(settings.backgroundVariation ?? 'daily') === 'preset' ? (
            <RadioGroup testID="settings-background-preset-group" accessibilityLabel={t('settings.backgroundVariationPreset')} style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {[0, 1, 2, 3].map((preset) => (
          ...
          <RadioGroup testID="settings-background-motion-group" accessibilityLabel={t('settings.backgroundMotion')} style={{ gap: 6 }}>
            <Text style={{ color: colors.textSecondary, fontSize: 14, lineHeight: 20, fontWeight: '700' }}>{t('settings.backgroundMotion')}</Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          ...
          <RadioGroup testID="settings-background-intensity-group" accessibilityLabel={t('settings.backgroundIntensity')} style={{ gap: 6 }}>
            <Text style={{ color: colors.textSecondary, fontSize: 14, lineHeight: 20, fontWeight: '700' }}>{t('settings.backgroundIntensity')}</Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
            {LANGUAGE_OPTIONS.map((item) => (
              <IslePressable key={item.id} haptic accessibilityLabel={item.label} accessibilityState={{ selected: settings.language === item.id }} onPress={() => void chooseLanguage(item.id)} style={settingsChipPressableStyle}>
                <IsleChip active={settings.language === item.id}>{item.label}{settings.language === item.id ? ' ✓' : ''}</IsleChip>
              </IslePressable>
            ))}
          </View>
        </MotiView>
      ...
      <AnimatePresence>
        {expandedGroups.data ? (
          ...
          >
            ...
            />
            <Text style={{ color: colors.textTertiary, fontSize: 14, fontWeight: '700', lineHeight: 20 }}>
      ...
      <AnimatePresence>
        ...
        {expandedGroups.diagnostics ? (
          ...
          >
            ...
            <View style={foldoutCardStyle(7)}>
              <Text style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{t('settings.runtimeLogFile')}</Text>
              <Text selectable style={{ color: colors.textSecondary, fontSize: 14, lineHeight: 20, fontWeight: '800' }}>
        ...
        {expandedGroups.governance ? (
          ...
          >
            <SettingsSection id="governance.transportMode"><SegmentedSetting
              label={t('settings.transportMode')}
              options={TRANSPORT_OPTIONS}
              value={settings.transportMode ?? 'auto'}
              onChange={(transportMode) => updateSettings({ transportMode })}
            /></SettingsSection>
            <SettingsSection id="governance.remoteCompactMode"><SegmentedSetting
            ...
            <View style={pairedFieldRowStyle}>
              <SettingsSection id="governance.remoteCompactThreshold"><SavedSettingsField session={fieldSession} settingKey="remoteCompactThreshold"
              ...
              /></SettingsSection>
              <SettingsSection id="governance.remoteCompactThresholdTokens"><SavedSettingsField session={fieldSession} settingKey="remoteCompactThresholdTokens"
            ...
            <SettingsSection id="governance.payloadPolicyMode"><SegmentedSetting
            ...
            /></SettingsSection>
            <View style={foldoutCardStyle(8)}>
              <Text style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{t('settings.policySummary')}</Text>
              ...
              <Text style={{ color: colors.textTertiary, fontSize: 14, lineHeight: 20, fontWeight: '800' }}>{t('settings.policySummaryNote')}</Text>
            </View>
            <SettingsSection id="governance.proxyMode"><SegmentedSetting
              label={t('settings.proxyMode')}
              options={PROXY_OPTIONS}
              value={settings.proxyMode ?? 'off'}
              onChange={(proxyMode) => updateSettings({ proxyMode })}
            /></SettingsSection>
            <SettingsSection id="governance.proxyBaseUrl"><SavedSettingsField session={fieldSession} settingKey="proxyBaseUrl"
            ...
            <View style={foldoutCardStyle(6)}>
              <Text style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{t('settings.compactExecutionPolicy')}</Text>
          ...
          <IsleButton label={t('preferences.agentWorkflow')} onPress={() => pushSettingsChildRoute('/settings/preferences', { section: 'workflow' })} />
          ...
          <MotiView
            ...
            style={foldoutCardStyle()}
          >
            <Text style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{t('settings.observabilitySinkSettings')}</Text>
            <SettingsSection id="governance.observabilitySinkMode"><SegmentedSetting
            ...
            /></SettingsSection>
            <SettingsSection id="governance.observabilitySinkTarget"><SegmentedSetting
            ...
            /></SettingsSection>
            <SettingsSection id="governance.observabilitySinkEndpointUrl"><SavedSettingsField session={fieldSession} settingKey="observabilitySinkEndpointUrl"
            ...
            /></SettingsSection>
            <SettingsSection id="governance.observabilitySinkHighFrequencyMode"><SegmentedSetting
            ...
            /></SettingsSection>
            <View style={pairedFieldRowStyle}>
              <SettingsSection id="governance.observabilitySinkAttributeLimit"><SavedSettingsField session={fieldSession} settingKey="observabilitySinkAttributeLimit"
                ...
                style={pairedFieldStyle}
              /></SettingsSection>
              <SettingsSection id="governance.observabilitySinkAttributeStringLimit"><SavedSettingsField session={fieldSession} settingKey="observabilitySinkAttributeStringLimit"
            ...
            <SettingsSection id="governance.observabilitySinkApiKey"><IsleField
              label={t('settings.observabilitySinkApiKey')}
              note={t('settings.observabilitySinkApiKeyNote')}
              inputProps={{
                value: observabilitySinkApiKeyDraft,
                editable: observabilityKeyLoaded && !savingObservabilitySinkApiKey,
          ...
          >
            <View style={pairedFieldRowStyle}>
              <SettingsSection id="governance.sessionConcurrencyLimit"><SavedSettingsField session={fieldSession} settingKey="sessionConcurrencyLimit"
                ...
                style={pairedFieldStyle}
              /></SettingsSection>
              <SettingsSection id="governance.sessionQueueTimeoutMs"><SavedSettingsField session={fieldSession} settingKey="sessionQueueTimeoutMs"
            ...
            <View style={foldoutCardStyle()}>
              <Text style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{t('settings.sessionAffinitySettings')}</Text>
              <IsleToggle
              ...
              />
              <SettingsSection id="governance.sessionAffinityTtlMs"><SavedSettingsField session={fieldSession} settingKey="sessionAffinityTtlMs"
            ...
            <View style={foldoutCardStyle()}>
              <Text style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{t('settings.retryTimeoutSettings')}</Text>
              <View style={pairedFieldRowStyle}>
                <SettingsSection id="governance.upstreamRequestTimeoutMs"><SavedSettingsField session={fieldSession} settingKey="upstreamRequestTimeoutMs"
                  ...
                  style={pairedFieldStyle}
                /></SettingsSection>
                <SettingsSection id="governance.upstreamMaxRetries"><SavedSettingsField session={fieldSession} settingKey="upstreamMaxRetries"
              ...
              />
              <View style={pairedFieldRowStyle}>
                <SettingsSection id="governance.upstreamCircuitBreakerFailureThreshold"><SavedSettingsField session={fieldSession} settingKey="upstreamCircuitBreakerFailureThreshold"
                  ...
                  style={pairedFieldStyle}
                /></SettingsSection>
                <SettingsSection id="governance.upstreamCircuitBreakerCooldownMs"><SavedSettingsField session={fieldSession} settingKey="upstreamCircuitBreakerCooldownMs"
          ...
          >
            <View style={foldoutCardStyle()}>
              <Text style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{t('settings.rectificationSettings')}</Text>
            ...
            <View style={foldoutCardStyle()}>
              <Text style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{t('settings.bedrockOptimizerSettings')}</Text>
              ...
              <SettingsSection id="governance.cacheTtl"><SegmentedSetting label={t('settings.cacheTtl')} options={CACHE_TTL_OPTIONS} value={settings.cacheTtl ?? 'default'} onChange={(cacheTtl) => updateSettings({ cacheTtl })} /></SettingsSection>
            </View>
          </MotiView>
          ) : null}
          <IsleDisclosure
          ...
          >
            ...
            />
            <SettingsSection id="governance.runtimeLogMaxBytes"><SavedSettingsField session={fieldSession} settingKey="runtimeLogMaxBytes"
              ...
              inputProps={{ keyboardType: 'number-pad' }}
            /></SettingsSection>
            <SettingsSection id="governance.providerAllowlist"><SavedSettingsField session={fieldSession} settingKey="providerAllowlist"
            ...
            /></SettingsSection>
            <SettingsSection id="governance.providerBlocklist"><SavedSettingsField session={fieldSession} settingKey="providerBlocklist"
              ...
              inputProps={{ autoCapitalize: 'none', autoCorrect: false, multiline: true, style: { minHeight: 48, maxHeight: 88 } }}
            /></SettingsSection>
            <SettingsSection id="governance.modelAllowlist"><SavedSettingsField session={fieldSession} settingKey="modelAllowlist"
              ...
              inputProps={{ autoCapitalize: 'none', autoCorrect: false, multiline: true, style: { minHeight: 48, maxHeight: 88 } }}
            /></SettingsSection>
            <SettingsSection id="governance.modelBlocklist"><SavedSettingsField session={fieldSession} settingKey="modelBlocklist"
        ...
        {expandedGroups.updates ? (
          ...
          >
            ...
            {updateProgressDetail ? (
              <View style={{ borderRadius: Math.min(colors.ui.radius.card, 8), padding: 10, backgroundColor: resolveSettingsFoldoutSurface(colors, colors.ui.liquidGlass, 'muted'), borderWidth: subtleBorderWidth, borderColor: resolveSettingsFoldoutBorder(colors, colors.ui.liquidGlass) }}>
                <Text style={{ color: colors.text, fontSize: 14, lineHeight: 20, fontWeight: '800' }}>{updateActionLabel}</Text>
                <IsleProgress
...
function SettingsInlineLabel({ title, detail }: { title: string; detail: string }) {
  ...
  return (
    <View style={{ paddingTop: 2 }}>
      ...
      <Text style={{ color: colors.textTertiary, fontSize: 14, lineHeight: 20, marginTop: 2, fontWeight: '500', includeFontPadding: false, textAlignVertical: 'center' }}>{detail}</Text>
    </View>
  )
}
    ...
    <View style={{ borderRadius: Math.min(colors.ui.radius.card, 8), padding: 10, backgroundColor: colors.ui.monet ? colors.ui.semantic.surface.muted : colors.ui.liquidGlass ? colors.ui.actionBar.itemBackground : colors.ui.semantic.surface.muted, borderWidth: colors.ui.monet ? 1 : StyleSheet.hairlineWidth, borderColor: colors.ui.monet ? colors.material.stroke : colors.ui.liquidGlass ? colors.ui.actionBar.itemBorder : colors.ui.semantic.chrome.border, gap: 9 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text style={{ color: colors.text, fontSize: 14, lineHeight: 20, fontWeight: '800', includeFontPadding: false, textAlignVertical: 'center' }}>{t('settings.runtimeRepairTasks')}</Text>
          <Text style={{ color: colors.textTertiary, fontSize: 14, lineHeight: 20, fontWeight: '800', marginTop: 2, includeFontPadding: false, textAlignVertical: 'center' }}>
    ...
    <IslePressable haptic accessibilityRole="switch" accessibilityLabel={description ? `${title}. ${description}` : title} accessibilityState={{ checked: active }} onPress={onPress} style={{ borderRadius: Math.min(colors.ui.radius.controlMiddle, 8) }}>
      ...
      >
        ...
        <View style={{ flex: 1, minWidth: 0, justifyContent: 'center' }}>
          <Text style={{ color: colors.text, fontSize: 15, lineHeight: 21, fontWeight: '700', includeFontPadding: false, textAlignVertical: 'center' }}>
            {title}
          </Text>
          {description ? (
  ...
  const cardRadius = radioExpression.shape === 'angular'
    ...
    : radioExpression.shape === 'soft'
      ...
      : radioExpression.shape === 'material'
        ...
        : 22
  return (
    <IslePressable haptic testID={testID} accessibilityRole="radio" accessibilityLabel={`${label}. ${detail}`} accessibilityState={{ checked: active }} aria-checked={active} onPress={onPress} style={{ flexGrow: 1, flexShrink: 1, flexBasis: compact ? '100%' : '47%', minWidth: 0, minHeight: 144 }}>
      <MotiView
        animate={{
        ...
        style={{
          ...
          // The appearance picker is an anchor while the rest of the UI changes.
          // Reserve the selected border and two description lines in every theme.
          padding: 10,
          borderWidth: 2,
          gap: 7,
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <Text style={{ flex: 1, minWidth: 0, color: colors.text, fontSize: 14, lineHeight: 20, fontWeight: '800' }}>
            {label}
          </Text>
          <ThemeRadioMark active={active} />
        </View>
        <Text style={{ minHeight: 30, color: colors.textSecondary, fontSize: 14, lineHeight: 20, fontWeight: '600' }}>
          {detail}
        </Text>
      </MotiView>
    </IslePressable>
...
function ThemeModeCard({ label, active, onPress, testID }: { label: string; active: boolean; onPress: (event: GestureResponderEvent) => void; testID: string }) {
  const { colors, canonicalThemeId } = useAppTheme()
  const accentMotion = useThemeMotion('accent')
  const radioExpression = resolveThemeComponentExpression(canonicalThemeId, 'radio')
  ...
  return (
    <IslePressable haptic testID={testID} accessibilityRole="radio" accessibilityLabel={label} accessibilityState={{ checked: active }} aria-checked={active} onPress={onPress} style={{ flex: 1, minWidth: 0, minHeight: themeModeCardHeight }}>
      ...
      >
        ...
        <Text style={{ color: active ? radioExpression.surface === 'boundary' ? colors.text : colors.ui.control.primaryForeground : colors.textSecondary, fontSize: 14, lineHeight: 20, fontWeight: '700', includeFontPadding: false, textAlignVertical: 'center' }}>
          {label}
        </Text>
      </MotiView>
    </IslePressable>
...
function SegmentedSetting<T extends string>({
  ...
  return (
    <RadioGroup accessibilityLabel={label}>
      <Text style={{ color: colors.text, fontSize: 14, lineHeight: 20, fontWeight: '700', marginBottom: 7, includeFontPadding: false, textAlignVertical: 'center' }}>{label}</Text>
...
function VersionRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <View style={{ minHeight: 28, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
      <Text style={{ color: colors.textTertiary, fontSize: 14, lineHeight: 20, fontWeight: '700', flexBasis: '34%', minWidth: 0, maxWidth: '42%', flexShrink: 1, includeFontPadding: false, textAlignVertical: 'center' }}>{label}</Text>
      <Text style={{ color: colors.text, fontSize: 14, lineHeight: 20, fontWeight: '800', flex: 1, minWidth: 0, includeFontPadding: false, textAlignVertical: 'center' }}>{value}</Text>
    </View>
  )
}
