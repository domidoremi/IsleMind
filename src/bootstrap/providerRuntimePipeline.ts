...
import {
  evaluateProviderPayloadRules as evaluatePayloadRules,
  type ProviderPayloadRuleResult as PayloadRuleResult,
} from '@/modules/providers'
import type { ProxyPolicyDecision } from '@/modules/providers'
import { resolveProxyPolicy } from '@/bootstrap/providerProxyPolicy'
import { mergeRuntimeAliasAccessPolicy, resolveProviderModelAccess, type AccessPolicyDecision } from '@/bootstrap/providerModelAccess'
import type { ProviderFailoverInput } from '@/modules/providers'
import { getProviderCompatibilityEvidenceForProvider } from '@/modules/providers'
import type { ProviderRouteContext, ProviderRouteResult as TargetProviderRouteResult, ProviderRouteAssembly, ProviderRouteAssemblyInput, ProviderTransportSelection as TransportSelection } from '@/modules/providers'
import type { ProviderConformanceResult } from '@/modules/providers'
...
import { getHostedProviderSupportIssue } from '@/bootstrap/providerPolicies'
import { isBedrockRuntimeProvider, prepareBedrockRuntimeInvokeModelRequest } from '@/modules/providers'
import { clampMaxTokens, optimizeRequestBody as optimizeProviderRequestBody } from '@/bootstrap/providerRequestPolicies'
import { resolveProviderStreamingPolicy } from '@/modules/providers'
import { emitRuntimeGovernanceTrace, logPayloadPolicy, logProviderCompatibility, logProviderConformance, logProviderRouteDecision, logProxyPolicy, logUpstreamRequest, runtimeLogOptions } from '@/bootstrap/providerRuntimeDiagnostics'
import { providerRuntimeError, type ProviderRuntimeError } from '@/modules/providers'
import { type ProviderRuntimeHealthView } from '@/modules/providers'
import type { ProviderRuntimeChatRequest, ProviderRuntimeTraceCallback } from '@/modules/providers'
import { providerRuntimeHealthRoute, resolveProviderRuntimeHealthView } from '@/bootstrap/providerRuntimeHealth'
export const PROVIDER_ROUTE_DECISION_SNAPSHOT_SCHEMA = 'islemind.provider-route-decision-snapshot.v1'

type ProviderRouteResult = TargetProviderRouteResult<ProviderConformanceResult>
...
export interface ProviderRouteDecisionSnapshot {
  ...
  conformance: {
    ...
    generationParameters?: ProviderRouteResult['conformance']['parameterDiagnostics']
    reasoningResolution: {
...
export interface ProviderRuntimeRouteResolver {
  (req: ProviderRuntimeChatRequest, context?: ProviderRouteContext, failover?: ProviderFailoverInput): ProviderRouteResult
}
...
export interface ProviderRuntimePipelineInput {
  ...
  assembleProviderRoute(input: ProviderRouteAssemblyInput): ProviderRouteAssembly
}
...
export interface ProviderRuntimePipelineReady {
  ...
  requestedModel: string
  upstreamModel: string
  effectiveReq: ProviderRuntimeChatRequest
  runtimeReq: ProviderRuntimeChatRequest
  credentialGroupId?: string
...
export async function prepareProviderRuntimePipeline(input: ProviderRuntimePipelineInput): Promise<ProviderRuntimePipelineResult> {
  ...
  bindProviderRuntimeCancellation(effectiveReq.signal, input.controller)
  throwIfProviderRuntimePreparationAborted(input.controller.signal)
  ...
  const runtimeReq: ProviderRuntimeChatRequest = {
    ...effectiveReq,
    provider: {
  ...
  const streamingPolicy = resolveProviderStreamingPolicy({
    provider: runtimeReq.provider,
    requested: effectiveReq.stream,
    modelSupportsStreaming: runtimeModelConfig.supportsStreaming,
  })
  const stream = streamingPolicy.stream
  ...
  const routeAssembly = input.assembleProviderRoute({
    provider: runtimeReq.provider,
  ...
  const headers = getHeaders(runtimeReq.provider, {
    remoteCompactEligible: runtimeReq.remoteCompactEligible === true,
    model: runtimeReq.model,
  })
...
function bindProviderRuntimeCancellation(signal: AbortSignal | undefined, controller: AbortController): void {
  if (!signal) return
  if (signal.aborted) {
...
function throwIfProviderRuntimePreparationAborted(signal: AbortSignal): void {
  if (!signal.aborted) return
  throw providerRuntimeCancellationReason(signal)
}
