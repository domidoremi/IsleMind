...
| 状態 | プレリリースの適格性検証中 |
| `1.1.2` の本番 APK 添付 | 未公開 |

`1.1.2` は適格性検証中です。GitHub のタグとリリースノートは本番ビルドの検証完了を意味しません。現在の Release に APK とチェックサムは添付されていません。開発・検証用 APK はソースからビルドし、本番 APK は検証済み Release の実際の添付ファイルを確認してください。

- [v1.1.2 のリリースノートを見る](https://github.com/domidoremi/IsleMind/releases/tag/v1.1.2)
- [すべての Releases と実際の添付ファイルを見る](https://github.com/domidoremi/IsleMind/releases)

### 1.1.2 の検証範囲

本版はソースのみのプレリリースで、インストール可能な 1.1.2 APK は公開していません。以下の 1.1.1 の端末・ビルド結果は過去の証拠であり、1.1.2 の成果物そのものの検証ではありません。

2026-09-21 に `com.islemind.app` の ARM64 Release APK（`1.1.1` / `126`、no-model）をローカル生成し、チェックサム、ソース入力との一致、16 KiB アラインメントを検証しました。本番署名がないため、候補 APK は**未署名でインストール不可**です。隔離した Android 12 M2007J3SC で C4 とアプリ全体のライフサイクル復旧に合格し、インストール済みの**デバッグ版**の私有ファイルハッシュも維持されました。永続ブラウザープロファイルの Web 検証も合格しましたが、本番署名版、他の端末、プライベートモードの認証ではありません。正規の署名、適用されるポリシー・ライセンス判断、公開承認が未完了のため、**本番リリース準備は NOT READY** です。[ローカル検証とビルド手順](README.md#web-persistence-qualification)を参照してください。
