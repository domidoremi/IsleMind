...
function run() {
  ...
  assert.match(module, /HeadlessJsTaskConfig\("IsleMindAgentExecutionLease", Arguments\.createMap\(\), 0L, true\)/)
  assert.match(module, /it\.ownerId == ownerId && it\.expiresAtElapsedMs > SystemClock\.elapsedRealtime\(\)/)
  assert.match(module, /if \(!active\) \{ pauseExecutionTimers\(\); finishTimerScope\(\); return \}/)
  assert.match(module, /executionTimers\.size >= 256/)
  assert.match(module, /ownsLiveLease\(\) && executionTimers\.remove\(key\) != null/)
  assert.match(module, /handler\.postDelayed\(callback, delayMs\.toLong\(\)\)/)
  assert.match(module.slice(module.indexOf('override fun invalidate')), /executionTimers\.clear\(\)/)
  assert.match(module, /HeadlessJsTaskContext\.getInstance\(context\)\.finishTask\(id\)/)
  assert.match(module.slice(module.indexOf('override fun invalidate')), /finishTimerScope\(\)/)
  assert.match(idle, /syncTimerScope\(\)/)
