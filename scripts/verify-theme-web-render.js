...
check(
  'ThemeId contract exposes five canonical families',
  /export type CanonicalThemeId = 'minimal' \| 'monet' \| 'material' \| 'liquid-glass' \| 'animal-island-ui'/.test(settingsTypeSource)
...
check('themeIds contains exactly five canonical families', /export const themeIds = THEME_FAMILIES/.test(colorsSource) && /CANONICAL_THEME_IDS = \['minimal', 'monet', 'material', 'liquid-glass', 'animal-island-ui'\]/.test(settingsTypeSource), 'theme ids are sourced from the canonical settings contract')
...
check('settings UI offers five canonical theme families', /id: 'minimal'/.test(settingsScreen) && /id: 'monet'/.test(settingsScreen) && /id: 'material'/.test(settingsScreen) && /id: 'liquid-glass'/.test(settingsScreen) && /id: 'animal-island-ui'/.test(settingsScreen), 'preferences must allow all canonical families')
check('appearance cards expose rendered radio semantics and compact layout', /function ThemeFamilyCard[\s\S]*?accessibilityRole="radio"[\s\S]*?accessibilityState=\{\{ checked: active \}\}[\s\S]*?aria-checked=\{active\}/.test(settingsScreen) && /function ThemeModeCard[\s\S]*?accessibilityRole="radio"[\s\S]*?accessibilityState=\{\{ checked: active \}\}[\s\S]*?aria-checked=\{active\}/.test(settingsScreen) && /function ThemeAccentSwatch[\s\S]*?accessibilityRole="radio"[\s\S]*?accessibilityState=\{\{ checked: active \}\}[\s\S]*?aria-checked=\{active\}/.test(settingsAccent) && /const actionCompact = width < 360/.test(settingsScreen) && /flexDirection: actionCompact \? 'column' : 'row'/.test(settingsScreen), 'appearance controls must expose checked state to both React Native accessibility and rendered web ARIA')
check('custom accent remains a checked radio choice', /activeCustomThemeAccent/.test(settingsAccent) && /settings-theme-accent-custom/.test(settingsAccent) && /disabled=\{!customValue && !savedCustom\}/.test(settingsAccent), 'a custom hexadecimal accent must not leave the accent radio group without a selected option')
check('app action policy accepts five canonical theme ids', /SETTINGS_THEME_FAMILIES = CANONICAL_THEME_IDS/.test(settingsActionContracts), 'assistant actions should use the canonical runtime ids')
check('tool schemas expose canonical family inputs only', /enum: \['minimal', 'monet', 'material', 'liquid-glass', 'animal-island-ui'\]/.test(builtinTools) && !/lime-road|material-3|material3|Legacy .*migrated/.test(builtinTools), 'assistant tools must reject retired theme ids at the schema boundary')
...
check('global CSS has all canonical theme fallback selectors', ['minimal', 'monet', 'material', 'liquid-glass', 'animal-island-ui'].every((family) => css.includes(`data-theme-id='${family}'`)), 'web fallback selectors must cover all canonical families')
...
for (const { locale, json } of localeFiles) {
  check(`${locale} locale has new theme labels`, Boolean(json.settings?.themeMinimal && json.settings?.themeMonet && json.settings?.themeMaterial && json.settings?.themeLiquidGlass && json.settings?.themeAnimalIsland), `${locale} settings labels should include all canonical theme families`)
  check(`${locale} locale has new theme descriptions`, Boolean(json.settings?.themeMinimalDescription && json.settings?.themeMonetDescription && json.settings?.themeMaterialDescription && json.settings?.themeLiquidGlassDescription && json.settings?.themeAnimalIslandDescription), `${locale} settings descriptions should include all canonical theme families`)
