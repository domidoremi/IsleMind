...
async function run() {
  ...
  assertSourceIncludes('src/bootstrap/runtimeDiagnostics.ts', 'RUNTIME_DIAGNOSTICS_PROVIDER_HEAVY_LIMIT', 'runtime diagnostics caps heavy provider scans')
  ...
  assertSourceIncludes('src/components/providers/ProviderSettingsContent.tsx', "await import('@/bootstrap/runtimeDiagnostics')", 'provider settings loads runtime diagnostics only after debounce and current-run admission')
  ...
  assertSourceIncludes('src/components/main/SettingsScreenContent.tsx', "await import('@/bootstrap/runtimeDiagnostics')", 'settings loads runtime diagnostics only when the diagnostics surface refreshes')
