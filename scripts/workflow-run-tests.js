...
async function testStructuredArtifactChatOutcomes(input) {
  ...
  for (const source of sources) {
    ...
    assert.match(resolution.patch.content, /structural audit \(advisory only\)/i)
    assert.ok(resolution.patch.content.includes(`qualityAudit.ok=${source.auditOk}`))
    assert.ok(!resolution.patch.content.includes(source.nextStep), 'model-facing audit output does not promote a suggested next step into an instruction')
