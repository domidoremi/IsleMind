...
function assertChatPresentationCatalog() {
  ...
  const controlOrbActionsSource = fs.readFileSync(path.join(root, 'src/components/chat/chatControlOrbActions.ts'), 'utf8')
  ...
  assert.ok(chatSetupWorkspaceSource.includes('actionLabel={undefined}'), 'the setup surface omits a duplicate provider action beside the composer')
  assert.ok(!chatSetupWorkspaceSource.includes('firstRunProviderSetupDescription'), 'the no-provider setup surface omits the onboarding paragraph')
  assert.ok(chatSetupWorkspaceSource.includes('actionHint={undefined}') && chatSetupWorkspaceSource.includes('glyph={undefined}'), 'the omitted setup action leaves no inaccessible hint or decorative provider glyph')
  ...
  assert.ok(chatSetupWorkspaceSource.includes('onAction={undefined}'), 'the setup surface does not restore the redundant provider button')
  ...
  assert.ok(chatWorkspaceConstantsSource.includes('CHAT_USES_FLOATING_CONTROL_ORB = true'), 'chat workspace layout enables the floating control orb')
  assert.ok(controlOrbActionsSource.includes("key: 'model'"), 'floating control orb exposes model selection')
  assert.ok(controlOrbActionsSource.includes("key: 'providers'"), 'floating control orb exposes provider settings')
  assert.ok(controlOrbActionsSource.includes("key: 'memory'"), 'floating control orb exposes memory review')
  assert.ok(controlOrbActionsSource.includes("key: 'settings'"), 'floating control orb exposes app settings')
