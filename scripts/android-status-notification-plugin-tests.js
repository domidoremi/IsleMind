...
const pluginIconPath = path.join(root, 'plugins', 'android-status-notification', 'ic_islemind_status.xml')
...
const nativeIconPath = path.join(root, 'android', 'app', 'src', 'main', 'res', 'drawable', 'ic_islemind_status.xml')
...
function run() {
  ...
  const pluginIconText = fs.readFileSync(pluginIconPath, 'utf8')
  ...
  const nativeIconText = fs.readFileSync(nativeIconPath, 'utf8')
  ...
  for (const permission of ['android.permission.POST_NOTIFICATIONS', 'android.permission.POST_PROMOTED_NOTIFICATIONS', 'android.permission.FOREGROUND_SERVICE', 'android.permission.FOREGROUND_SERVICE_DATA_SYNC']) {
  ...
  assert.ok(pluginText.includes('AndroidStatusNotificationService'), 'Android status notification plugin must declare the foreground service.')
  assert.ok(pluginText.includes('android:foregroundServiceType'), 'Android status notification plugin must declare the service foreground type.')
  assert.ok(manifestText.includes('AndroidStatusNotificationService'), 'AndroidManifest must declare AndroidStatusNotificationService.')
  assert.ok(manifestText.includes('android:foregroundServiceType="dataSync"'), 'AndroidManifest must mark the status service as dataSync.')
  ...
  for (const snippet of [
    ...
    'notificationPosted',
    'R.drawable.ic_islemind_status',
    'notificationAccent(payload.state)',
  ...
  assert.ok(pluginText.includes('ic_islemind_status.xml'), 'Android status notification plugin must install the monochrome status icon.')
  assert.match(pluginIconText, /android:fillColor="#FFFFFFFF"/, 'Status icon source must be a monochrome white notification glyph.')
  assert.equal(nativeIconText, pluginIconText, 'Generated native status icon must match the plugin source.')
