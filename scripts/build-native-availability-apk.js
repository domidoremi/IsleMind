#!/usr/bin/env node
// Builds a side-by-side qualification APK. Never installs or publishes anything.
...
const { qualificationProfile, configureQualificationGradle, configureQualificationManifest } = require('./native-availability-qualification')

const root = path.resolve(__dirname, '..')
const args = process.argv.slice(2)
const option = (name) => args[args.indexOf(name) + 1]
const profile = qualificationProfile(args.includes('--network'))
const pkg = profile.package
const stage = args.includes('--stage') && path.resolve(option('--stage'))
assert(stage && stage.toLowerCase() !== root.toLowerCase() && !stage.toLowerCase().startsWith(root.toLowerCase() + path.sep), '--stage must be outside the repository')
...
const inheritedKeys = new Set(['SYSTEMROOT', 'WINDIR', 'COMSPEC', 'PATHEXT', 'TEMP', 'TMP', 'LOCALAPPDATA',
  'APPDATA', 'USERPROFILE', 'HOMEDRIVE', 'HOMEPATH', 'HOME', 'ANDROID_HOME', 'ANDROID_SDK_ROOT', 'GRADLE_USER_HOME',
  'NUMBER_OF_PROCESSORS', 'PROCESSOR_ARCHITECTURE'])
const buildEnv = { ...Object.fromEntries(Object.entries(process.env).filter(([key]) => inheritedKeys.has(key.toUpperCase()))),
  JAVA_HOME: javaHome, [pathKey]: buildPath, EXPO_NO_DOTENV: '1', NODE_ENV: 'production',
  CMAKE_BUILD_PARALLEL_LEVEL: '1', JAVA_TOOL_OPTIONS: '--enable-native-access=ALL-UNNAMED' }
const marker = path.join(stage, 'stage9-build.json')
assert(!fs.existsSync(stage), 'Use a new staging directory; existing native/signing files must not enter qualification')
fs.mkdirSync(stage, { recursive: true })
const hash = (file) => crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex')
const receipt = { schema: 'islemind.native-availability-build.v1', package: pkg, source: root, stage, profile: profile.name,
  buildType: profile.buildType, productionRelease: false,
  createdAt: new Date().toISOString(), node: process.execPath, javaHome, toolchain,
  instrumentation: [profile.internet ? 'explicit network qualification permission' : 'offline permission overlay', 'separate package, scheme and label', 'backup disabled',
    'optimized JS/Hermes with dev support disabled; dedicated debuggable qualification build',
...
for (const item of ['app', 'src', 'assets', 'plugins', 'patches', 'public', 'scripts',
...
app.expo.name = `IsleMind Qualification (${profile.name})`
app.expo.scheme = profile.scheme
app.expo.android.package = pkg
app.expo.version = '1.1.0-qualification'
app.expo.android.versionCode = 9001
fs.writeFileSync(path.join(stage, 'app.json'), JSON.stringify(app, null, 2))
edit('android/app/build.gradle', text => configureQualificationGradle(text, profile))
edit('android/app/src/main/AndroidManifest.xml', text => configureQualificationManifest(text, profile))
edit('android/app/src/main/res/values/strings.xml', text => text.replace(/(<string name="app_name">)[^<]+/, `$1IsleMind Qualification (${profile.name})`))
...
if (!args.includes('--prepare-only')) {
  ...
  const gradleArgs = ['-classpath', path.join(stage, 'android/gradle/wrapper/gradle-wrapper.jar'),
    'org.gradle.wrapper.GradleWrapperMain', 'app:assembleQualification', '-PreactNativeArchitectures=arm64-v8a',
    '--console=plain', '--max-workers=1', '--no-daemon', '-Dorg.gradle.parallel=false']
  const result = spawnSync(java, gradleArgs, {
  ...
  receipt.apk = path.join(stage, 'android/app/build/outputs/apk/qualification/app-qualification.apk')
