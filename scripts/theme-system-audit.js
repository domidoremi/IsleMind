...
const materialLightCss = cssFallbackBlock('material', 'light')
const materialDarkCss = cssFallbackBlock('material', 'dark')
...
check('Liquid Glass has one explicit capability/fallback contract', /interface GlassCapabilityContract/.test(glassSurface) && /resolveGlassCapability/.test(glassSurface) && /targetedBackdropSupported/.test(glassSurface) && /capability\.realtimeBlurSupported/.test(glassSurface), 'platform/version checks and blur fallback policy must be centralized and fail closed')
...
check('settings owns canonical-only family normalization', /CANONICAL_THEME_IDS as readonly string\[\]/.test(settingsContracts) && /normalizeThemeFamilyValue\(value\)/.test(settingsAppearance) && /normalizeThemeFamilyValue\(value\) \?\? DEFAULT_THEME_ID/.test(colors) && !/THEME_ID_ALIASES/.test(settingsContracts + settingsAppearance + colors), 'theme resolution must consume one canonical-only Settings contract')
check('settings migration rewrites stale ids to Minimal', /normalizeThemeId\(rawSettings\.themeId\)/.test(settingsStore) && /themeIdMigrated/.test(settingsStore) && /themeId: normalizedThemeId/.test(settingsStore), 'retired or corrupt persisted ids should be rewritten after load')
...
const criticalFallbacks = [
  ['minimal light', cssFallbackBlock('minimal', 'light'), { '--color-primary': '#1f5b50', '--color-semanticControlBackground': '#1f5b50', '--color-semanticChromeToolbar': '#d4ede5' }],
  ['minimal dark', cssFallbackBlock('minimal', 'dark'), { '--color-primary': '#a8dccb', '--color-semanticControlBackground': '#a8dccb', '--color-semanticChromeToolbar': '#174a3e' }],
  ['monet light', cssFallbackBlock('monet', 'light'), { '--color-primary': '#2f6e68', '--color-semanticControlBackground': '#2f6e68', '--color-semanticChromeToolbar': '#cfe8df' }],
  ['monet dark', cssFallbackBlock('monet', 'dark'), { '--color-primary': '#9dd6c8', '--color-semanticControlBackground': '#9dd6c8', '--color-semanticChromeToolbar': '#28564a' }],
  ['material light', cssFallbackBlock('material', 'light'), { '--color-primary': '#365f86', '--color-semanticControlBackground': '#365f86', '--color-semanticChromeToolbar': '#d8e7f5' }],
  ['material dark', cssFallbackBlock('material', 'dark'), { '--color-primary': '#a6c8ea', '--color-semanticControlBackground': '#a6c8ea', '--color-semanticChromeToolbar': '#234b6d' }],
  ['liquid-glass light', cssFallbackBlock('liquid-glass', 'light'), { '--color-primary': '#155e87', '--color-semanticControlBackground': '#155e87', '--color-semanticChromeToolbar': 'rgba(139, 205, 237, 0.56)' }],
  ['liquid-glass dark', cssFallbackBlock('liquid-glass', 'dark'), { '--color-primary': '#8ed0f0', '--color-semanticControlBackground': '#8ed0f0', '--color-semanticChromeToolbar': 'rgba(55, 113, 145, 0.72)' }],
]
