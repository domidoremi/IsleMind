...
check('markdown compatibility surfaces match semantic canvas layers', /const markdownLight[\s\S]*?surface: '#F6F8FA'[\s\S]*?surfaceSecondary: '#FFFFFF'[\s\S]*?surfaceTertiary: '#EFF2F5'/.test(colors) && /const markdownDark[\s\S]*?surface: '#0D1117'[\s\S]*?surfaceSecondary: '#161B22'[\s\S]*?surfaceTertiary: '#1F242C'/.test(colors), 'root, safe-area, and legacy surfaces must not expose Minimal colors around Markdown pages')
