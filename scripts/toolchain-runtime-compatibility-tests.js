...
const {
  ...
  createExternalAgentRuntimePolicy,
...
async function run() {
  ...
  const externalDesktopRuntime = runtimes.find((runtime) => runtime.id === 'desktop-local')
  assert.ok(externalDesktopRuntime, 'external runtime fixtures resolve a non-Android desktop runtime')
  const externalDesktopTaskRequest = createToolchainTaskRequest({
    manifest: tool('islemind.skill.validate'),
    runtime: externalDesktopRuntime,
    taskId: 'task-external-agent-unrelated-fixture',
    projectId: 'islemind',
    payload: { instruction: 'bounded-at-source' },
    now: 2000000000000,
  })
  const externalDesktopTaskRecord = createToolchainTaskRecord({
    task: externalDesktopTaskRequest,
    ttlMs: 60000,
    now: 2000000000000,
  })
  const externalDesktopStartedTask = transitionToolchainTask(externalDesktopTaskRecord, 'running', {
    now: 2000000001000,
    reason: 'Desktop runtime accepted unrelated durable task.',
  })
  assert.equal(externalDesktopStartedTask.ok, true, 'unrelated desktop fixture task enters the running state')
  assert.equal(externalDesktopStartedTask.task.runtimeId, externalDesktopRuntime.id, 'unrelated desktop fixture task binds the desktop runtime')
  const externalCodexCliTaskRecord = Object.freeze({
    ...externalDesktopTaskRecord,
    taskId: 'task-external-agent-codex-cli-fixture',
    toolId: EXTERNAL_AGENT_RUNTIME_TASK_TOOL_IDS['codex-cli'],
  })
  const externalCodexCliStartedTask = transitionToolchainTask(externalCodexCliTaskRecord, 'running', {
    now: 2000000001000,
    reason: 'Desktop runtime accepted Codex CLI session task.',
  })
  assert.equal(externalCodexCliStartedTask.ok, true, 'Codex CLI fixture task enters the running state')
  assert.equal(externalCodexCliStartedTask.task.runtimeId, externalDesktopRuntime.id, 'Codex CLI fixture task binds the desktop runtime')
  const externalCodexDesktopTaskRecord = Object.freeze({
    ...externalDesktopTaskRecord,
    taskId: 'task-external-agent-codex-desktop-fixture',
    toolId: EXTERNAL_AGENT_RUNTIME_TASK_TOOL_IDS['codex-desktop'],
  })
  const externalCodexDesktopStartedTask = transitionToolchainTask(externalCodexDesktopTaskRecord, 'running', {
    now: 2000000001000,
    reason: 'Desktop runtime accepted Codex Desktop session task.',
  })
  assert.equal(externalCodexDesktopStartedTask.ok, true, 'Codex Desktop fixture task enters the running state')
  const externalClaudeCodeTaskRecord = Object.freeze({
    ...externalDesktopTaskRecord,
    taskId: 'task-external-agent-claude-code-fixture',
    toolId: EXTERNAL_AGENT_RUNTIME_TASK_TOOL_IDS['claude-code'],
  })
  const externalClaudeCodeStartedTask = transitionToolchainTask(externalClaudeCodeTaskRecord, 'running', {
    now: 2000000001000,
    reason: 'Desktop runtime accepted Claude Code session task.',
  })
  assert.equal(externalClaudeCodeStartedTask.ok, true, 'Claude Code fixture task enters the running state')
  const externalGrokCliTaskRecord = Object.freeze({
    ...externalDesktopTaskRecord,
    taskId: 'task-external-agent-grok-cli-fixture',
    toolId: EXTERNAL_AGENT_RUNTIME_TASK_TOOL_IDS['grok-cli'],
  })
  const externalGrokCliStartedTask = transitionToolchainTask(externalGrokCliTaskRecord, 'running', {
    now: 2000000001000,
    reason: 'Desktop runtime accepted Grok CLI session task.',
  })
  assert.equal(externalGrokCliStartedTask.ok, true, 'Grok CLI fixture task enters the running state')

  assert.equal(EXTERNAL_AGENT_RUNTIME_DESCRIPTOR_SCHEMA, 'islemind.external-agent-runtime-descriptor.v1', 'external runtime descriptors use a versioned schema')
  assert.equal(EXTERNAL_AGENT_RUNTIME_SESSION_REQUEST_SCHEMA, 'islemind.external-agent-runtime-session-request.v1', 'external runtime requests use a versioned schema')
  assert.equal(EXTERNAL_AGENT_RUNTIME_SESSION_OUTCOME_SCHEMA, 'islemind.external-agent-runtime-session-outcome.v1', 'external runtime outcomes use a versioned schema')
  assert.deepEqual(EXTERNAL_AGENT_RUNTIME_PRODUCTS, ['codex-cli', 'codex-desktop', 'claude-code', 'grok-cli'], 'external runtime products are closed and stable')
  assert.deepEqual(EXTERNAL_AGENT_RUNTIME_PROTOCOLS, ['codex-app-server', 'claude-code-stream-json', 'grok-acp'], 'external runtime protocols are closed and stable')
  assert.deepEqual(EXTERNAL_AGENT_RUNTIME_REQUIRED_CAPABILITIES, ['cli', 'task.run'], 'external runtime sessions require CLI and task authority')
  assert.deepEqual(EXTERNAL_AGENT_RUNTIME_TASK_TOOL_IDS, {
  ...
  }, 'external runtime products map to closed version-independent task capability identities')
  assert.equal(Object.isFrozen(EXTERNAL_AGENT_RUNTIME_PRODUCTS), true, 'external runtime products cannot be expanded by JavaScript consumers')
  assert.equal(Object.isFrozen(EXTERNAL_AGENT_RUNTIME_PROTOCOLS), true, 'external runtime protocols cannot be expanded by JavaScript consumers')
  assert.equal(Object.isFrozen(integrationsPublicApi.EXTERNAL_AGENT_RUNTIME_SESSION_IDENTITY_KINDS), true, 'external runtime session identity kinds cannot be expanded by JavaScript consumers')
  assert.equal(Object.isFrozen(EXTERNAL_AGENT_RUNTIME_REQUIRED_CAPABILITIES), true, 'external runtime required capabilities cannot be expanded by JavaScript consumers')
  assert.equal(Object.isFrozen(EXTERNAL_AGENT_RUNTIME_TASK_TOOL_IDS), true, 'external runtime task capability identities cannot be expanded by JavaScript consumers')
  assert.equal(integrationsPublicApi.EXTERNAL_AGENT_RUNTIME_TASK_TOOL_IDS, EXTERNAL_AGENT_RUNTIME_TASK_TOOL_IDS, 'external runtime task capability identities are exported through the Integrations public API')
  assert.equal(integrationsPublicApi.createExternalAgentRuntimePolicy, createExternalAgentRuntimePolicy, 'external runtime policy is exported only through the Integrations public API')

  const externalRuntimeDescriptors = [
    {
      schema: EXTERNAL_AGENT_RUNTIME_DESCRIPTOR_SCHEMA,
      id: 'codex-cli-0.145.0',
      product: 'codex-cli',
      name: 'Codex CLI',
      version: '0.145.0',
      ...
      supportsResume: true,
      requiredRuntimeCapabilities: ['cli', 'task.run'],
    },
    {
      schema: EXTERNAL_AGENT_RUNTIME_DESCRIPTOR_SCHEMA,
      id: 'codex-desktop-0.146.0-alpha.3.1',
  ...
  let externalRuntimeNow = 2000000002000
  const externalRuntimePolicy = createExternalAgentRuntimePolicy({
    isTrustedRuntimeSnapshot: (input) => runtimeSnapshotPolicy.isTrustedRuntimeSnapshot(input),
    isTrustedTaskRecord: (input) => TOOLCHAIN_RUNTIME_REPORT_TRUST_POLICY.isTrustedRuntimeReportApplicationTask(input),
    now: () => externalRuntimeNow,
  })
  assert.equal(TOOLCHAIN_RUNTIME_REPORT_TRUST_POLICY.isTrustedRuntimeReportApplicationTask(externalDesktopStartedTask.task), true, 'unrelated desktop fixture remains a trusted running task record')
  assert.equal(TOOLCHAIN_RUNTIME_REPORT_TRUST_POLICY.isTrustedRuntimeReportApplicationTask(externalCodexCliTaskRecord), true, 'Codex CLI queued task capability record is trusted')
  assert.equal(TOOLCHAIN_RUNTIME_REPORT_TRUST_POLICY.isTrustedRuntimeReportApplicationTask(externalCodexCliStartedTask.task), true, 'Codex CLI running task capability record is trusted')
  assert.equal(TOOLCHAIN_RUNTIME_REPORT_TRUST_POLICY.isTrustedRuntimeReportApplicationTask(externalCodexDesktopStartedTask.task), true, 'Codex Desktop running task capability record is trusted')
  assert.equal(TOOLCHAIN_RUNTIME_REPORT_TRUST_POLICY.isTrustedRuntimeReportApplicationTask(externalClaudeCodeStartedTask.task), true, 'Claude Code running task capability record is trusted')
  assert.equal(TOOLCHAIN_RUNTIME_REPORT_TRUST_POLICY.isTrustedRuntimeReportApplicationTask(externalGrokCliStartedTask.task), true, 'Grok CLI running task capability record is trusted')

  const externalRuntimeDescriptorInputsBeforeAdmission = JSON.stringify(externalRuntimeDescriptors)
  const admittedExternalDescriptors = externalRuntimeDescriptors.map((descriptor) => (
    externalRuntimePolicy.admitExternalAgentRuntimeDescriptor(descriptor)
  ))
  assert.equal(admittedExternalDescriptors.every((result) => result.ok), true, 'all four supported external runtime products have valid descriptors')
  assert.deepEqual(
    [...new Set(admittedExternalDescriptors.map((result) => result.descriptor.protocol))],
    ['codex-app-server', 'claude-code-stream-json', 'grok-acp'],
    'four product descriptors resolve to exactly three vendor protocols'
  )
  assert.equal(admittedExternalDescriptors.every((result) => Object.isFrozen(result.descriptor)), true, 'admitted external runtime descriptors are immutable')
  assert.equal(admittedExternalDescriptors.every((result) => Object.isFrozen(result.descriptor.requiredRuntimeCapabilities)), true, 'admitted external runtime capability evidence is immutable')
  assert.equal(JSON.stringify(externalRuntimeDescriptors), externalRuntimeDescriptorInputsBeforeAdmission, 'external runtime descriptor admission does not mutate caller-owned descriptor data')
  assert.equal(externalRuntimeDescriptors.every((descriptor) => !Object.isFrozen(descriptor) && !Object.isFrozen(descriptor.requiredRuntimeCapabilities)), true, 'external runtime descriptor admission freezes only normalized copies')

  const mismatchedExternalProtocol = externalRuntimePolicy.admitExternalAgentRuntimeDescriptor({
    ...externalRuntimeDescriptors[2],
    protocol: 'codex-app-server',
  })
  assert.equal(mismatchedExternalProtocol.ok, false, 'external runtime descriptors reject a valid protocol assigned to the wrong product')
  assert.equal(mismatchedExternalProtocol.code, 'protocol_mismatch', 'product/protocol mismatches fail with protocol_mismatch')

  const externalDescriptorInjectionSecret = 'sk-external-descriptor-injection-secret'
  const injectedExternalDescriptor = externalRuntimePolicy.admitExternalAgentRuntimeDescriptor({
    ...externalRuntimeDescriptors[0],
    command: 'codex app-server',
    executablePath: 'C:\\Program Files\\Codex\\codex.exe',
    apiKey: externalDescriptorInjectionSecret,
  })
  assert.equal(injectedExternalDescriptor.ok, false, 'external runtime descriptors reject command, path, and credential fields')
  assert.equal(injectedExternalDescriptor.code, 'invalid_descriptor', 'descriptor field injection fails closed')
  assert.equal(JSON.stringify(injectedExternalDescriptor).includes(externalDescriptorInjectionSecret), false, 'descriptor rejection never reflects injected credential material')

  const unsafeExternalDescriptorName = externalRuntimePolicy.admitExternalAgentRuntimeDescriptor({
    ...externalRuntimeDescriptors[0],
    name: 'node --eval forged',
  })
  assert.equal(unsafeExternalDescriptorName.ok, false, 'external runtime descriptors reject command-shaped display text')

  const unrelatedExternalTask = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalDesktopStartedTask.task,
    operation: 'open',
    instruction: 'Reject an unrelated trusted task capability.',
  })
  assert.equal(unrelatedExternalTask.ok, false, 'external sessions reject unrelated trusted task capabilities')
  assert.equal(unrelatedExternalTask.code, 'task_tool_mismatch', 'unrelated trusted task capabilities fail with task_tool_mismatch')

  const crossProductExternalTask = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalClaudeCodeStartedTask.task,
    operation: 'open',
    instruction: 'Reject authority issued for another external runtime product.',
  })
  assert.equal(crossProductExternalTask.ok, false, 'external sessions reject another product task capability')
  assert.equal(crossProductExternalTask.code, 'task_tool_mismatch', 'cross-product task capabilities fail with task_tool_mismatch')

  const externalOpen = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'open',
    instruction: 'Inspect the current workspace and report the relevant evidence.',
  })
  assert.equal(externalOpen.ok, true, 'a trusted running task can create a Codex CLI session request')
  assert.equal(externalOpen.request.descriptorId, externalRuntimeDescriptors[0].id, 'external request binds the exact descriptor ID')
  assert.equal(externalOpen.request.runtimeId, externalDesktopRuntime.id, 'external request binds the exact runtime ID')
  assert.equal(externalOpen.request.taskId, externalCodexCliStartedTask.task.taskId, 'external request binds the exact durable task ID')
  assert.equal(externalOpen.request.operation, 'open', 'external request records open semantics explicitly')
  assert.equal(Object.isFrozen(externalOpen.request), true, 'external session requests are immutable')
  assert.deepEqual(Object.keys(externalOpen.request).sort(), [
    'createdAt', 'descriptorId', 'instruction', 'operation', 'protocol', 'requestId', 'runtimeId', 'schema', 'taskId',
  ], 'external open request v1 wire shape remains unchanged')

  let mismatchedAuthorityPortCalls = 0
  const mismatchedAuthorityExecution = await externalRuntimePolicy.executeExternalAgentRuntimeSession({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalClaudeCodeStartedTask.task,
    request: externalOpen.request,
    signal: new AbortController().signal,
    port: {
      open: async () => {
        mismatchedAuthorityPortCalls += 1
        return undefined
      },
      resume: async () => {
        mismatchedAuthorityPortCalls += 1
  ...
  assert.equal(mismatchedAuthorityExecution.ok, false, 'execution rechecks the exact external product task capability')
  assert.equal(mismatchedAuthorityExecution.code, 'task_tool_mismatch', 'execution task capability drift fails with task_tool_mismatch')
  assert.equal(mismatchedAuthorityPortCalls, 0, 'mismatched external product authority performs no port work')

  const externalRuntimeBaseNow = externalRuntimeNow
  const externalTaskExpiresAt = externalCodexCliStartedTask.task.expiresAt
  assert.equal(typeof externalTaskExpiresAt, 'number', 'external runtime fixture carries explicit task expiry evidence')
  externalRuntimeNow = externalTaskExpiresAt - 1
  const beforeExpiryExternalRequest = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'open',
    instruction: 'Admit immediately before the durable task expires.',
  })
  assert.equal(beforeExpiryExternalRequest.ok, true, 'external session authority remains valid immediately before task expiry')
  externalRuntimeNow = externalTaskExpiresAt
  const atExpiryExternalRequest = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'open',
    instruction: 'Reject exactly when the durable task expires.',
  })
  assert.equal(atExpiryExternalRequest.ok, false, 'external session authority expires at the exact task TTL boundary')
  assert.equal(atExpiryExternalRequest.code, 'task_expired', 'exact task expiry fails with task_expired')
  externalRuntimeNow = externalTaskExpiresAt + 1
  const afterExpiryExternalRequest = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'open',
    instruction: 'Reject after the durable task expires.',
  })
  assert.equal(afterExpiryExternalRequest.ok, false, 'external session authority remains revoked after task expiry')
  assert.equal(afterExpiryExternalRequest.code, 'task_expired', 'post-expiry request creation fails with task_expired')

  let expiredExternalPortCalls = 0
  const expiredExecutionController = new AbortController()
  const expiredExternalExecution = await externalRuntimePolicy.executeExternalAgentRuntimeSession({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    request: beforeExpiryExternalRequest.request,
    signal: expiredExecutionController.signal,
    port: {
      open: async () => {
        expiredExternalPortCalls += 1
        return undefined
      },
      resume: async () => {
        expiredExternalPortCalls += 1
  ...
  assert.equal(expiredExternalExecution.ok, false, 'task expiry is rechecked at the external execution boundary')
  assert.equal(expiredExternalExecution.code, 'task_expired', 'expired external execution fails with task_expired')
  assert.equal(expiredExternalPortCalls, 0, 'expired external execution performs no port work')
  externalRuntimeNow = externalRuntimeBaseNow

  const externalRequestInputSecret = 'sk-external-request-input-secret'
  const injectedExternalRequest = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'open',
    instruction: 'Inspect the current workspace.',
    command: `codex app-server --api-key ${externalRequestInputSecret}`,
  })
  assert.equal(injectedExternalRequest.ok, false, 'external session request creation rejects extra command metadata')
  assert.equal(injectedExternalRequest.code, 'operation_mismatch', 'extra request fields fail closed before authority is minted')
  assert.equal(JSON.stringify(injectedExternalRequest).includes(externalRequestInputSecret), false, 'request rejection never reflects injected command secrets')

  const credentialInstruction = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'open',
    instruction: 'Use apiKey=sk-external-instruction-secret to continue.',
  })
  assert.equal(credentialInstruction.ok, false, 'external session instructions reject credential-shaped values')
  assert.equal(credentialInstruction.code, 'invalid_instruction', 'credential-shaped instructions fail closed before dispatch')

  const forgedExternalRuntimeSecret = 'sk-external-runtime-injection-secret'
  const forgedExternalRuntimeRequest = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: {
      ...externalDesktopRuntime,
      command: `codex app-server --token ${forgedExternalRuntimeSecret}`,
    },
    task: externalCodexCliStartedTask.task,
    operation: 'open',
    instruction: 'Inspect the current workspace.',
  })
  assert.equal(forgedExternalRuntimeRequest.ok, false, 'external session authority rejects forged runtime metadata')
  assert.equal(forgedExternalRuntimeRequest.code, 'runtime_unavailable', 'forged runtime metadata fails closed as unavailable')
  assert.equal(JSON.stringify(forgedExternalRuntimeRequest).includes(forgedExternalRuntimeSecret), false, 'runtime rejection never reflects forged secrets')

  const mismatchedExternalTask = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: { ...externalCodexCliStartedTask.task, runtimeId: 'termux-local', runtimeKind: 'termux' },
    operation: 'open',
    instruction: 'Inspect the current workspace.',
  })
  assert.equal(mismatchedExternalTask.ok, false, 'external session authority rejects a task bound to another runtime')
  assert.equal(mismatchedExternalTask.code, 'runtime_mismatch', 'task/runtime identity drift fails with runtime_mismatch')

  const queuedExternalTask = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliTaskRecord,
    operation: 'open',
    instruction: 'Inspect the current workspace.',
  })
  assert.equal(queuedExternalTask.ok, false, 'external sessions cannot start from queued task records')
  assert.equal(queuedExternalTask.code, 'task_not_running', 'non-running trusted tasks fail with task_not_running')

  const missingExternalCapability = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: { ...externalDesktopRuntime, capabilities: externalDesktopRuntime.capabilities.filter((capability) => capability !== 'cli') },
    task: externalCodexCliStartedTask.task,
    operation: 'open',
    instruction: 'Inspect the current workspace.',
  })
  assert.equal(missingExternalCapability.ok, false, 'external sessions require every declared runtime capability')
  assert.equal(missingExternalCapability.code, 'capability_missing', 'missing runtime capabilities fail closed')

  const externalSessionOnTermux = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[1],
    runtime: cancelRuntime,
    task: startedTask.task,
    operation: 'open',
    instruction: 'Inspect the current workspace.',
  })
  assert.equal(externalSessionOnTermux.ok, false, 'Termux is rejected as an Android-hosted external runtime')
  assert.equal(externalSessionOnTermux.code, 'runtime_unavailable', 'Termux external sessions fail closed as unavailable')

  const codexDesktopOpen = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[1],
    runtime: externalDesktopRuntime,
    task: externalCodexDesktopStartedTask.task,
    operation: 'open',
    instruction: 'Inspect the desktop workspace through the admitted contract.',
  })
  assert.equal(codexDesktopOpen.ok, true, 'Codex Desktop contract admission succeeds on the matching desktop runtime')

  const claudeCodeOpenInput = {
    descriptor: externalRuntimeDescriptors[2],
    runtime: externalDesktopRuntime,
    task: externalClaudeCodeStartedTask.task,
    operation: 'open',
    instruction: 'Inspect the workspace through the matching Claude Code contract.',
  }
  const claudeCodeOpenInputBeforeAdmission = JSON.stringify(claudeCodeOpenInput)
  const claudeCodeOpen = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest(claudeCodeOpenInput)
  assert.equal(claudeCodeOpen.ok, true, 'Claude Code contract admission succeeds with its matching durable task capability')
  assert.equal(claudeCodeOpen.request.protocol, 'claude-code-stream-json', 'Claude Code admission binds the declared stream-json protocol')
  assert.equal(claudeCodeOpen.request.taskId, externalClaudeCodeStartedTask.task.taskId, 'Claude Code admission binds the exact durable task ID')
  assert.equal(JSON.stringify(claudeCodeOpenInput), claudeCodeOpenInputBeforeAdmission, 'Claude Code request admission does not mutate caller-owned input')
  assert.equal(Object.isFrozen(claudeCodeOpenInput), false, 'Claude Code request admission does not freeze the caller-owned input wrapper')

  const grokCliOpenInput = {
    descriptor: externalRuntimeDescriptors[3],
    runtime: externalDesktopRuntime,
    task: externalGrokCliStartedTask.task,
    operation: 'open',
    instruction: 'Inspect the workspace through the matching Grok ACP contract.',
  }
  const grokCliOpenInputBeforeAdmission = JSON.stringify(grokCliOpenInput)
  const grokCliOpen = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest(grokCliOpenInput)
  assert.equal(grokCliOpen.ok, true, 'Grok CLI contract admission succeeds with its matching durable task capability')
  assert.equal(grokCliOpen.request.protocol, 'grok-acp', 'Grok CLI admission binds the declared ACP protocol')
  assert.equal(grokCliOpen.request.taskId, externalGrokCliStartedTask.task.taskId, 'Grok CLI admission binds the exact durable task ID')
  assert.equal(JSON.stringify(grokCliOpenInput), grokCliOpenInputBeforeAdmission, 'Grok CLI request admission does not mutate caller-owned input')
  assert.equal(Object.isFrozen(grokCliOpenInput), false, 'Grok CLI request admission does not freeze the caller-owned input wrapper')

  externalRuntimeNow += 1
  const codexThreadCorrelation = { kind: 'codex-thread', id: 'thread-019f-runtime-fixture' }
  const externalResume = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'resume',
    instruction: 'Continue from the caller-supplied vendor session correlation.',
    vendorSession: codexThreadCorrelation,
  })
  assert.equal(externalResume.ok, true, 'resume requires an explicit caller-supplied vendor-session correlation identity')
  assert.deepEqual(externalResume.request.vendorSession, codexThreadCorrelation, 'resume request preserves the exact vendor-session correlation identity')
  assert.equal(Object.isFrozen(externalResume.request.vendorSession), true, 'vendor-session correlation identity is cloned and frozen')

  const missingResumeIdentity = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'resume',
    instruction: 'Continue without correlation identity.',
  })
  assert.equal(missingResumeIdentity.ok, false, 'resume cannot silently become a new session when identity evidence is absent')

  const wrongResumeIdentity = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'resume',
    instruction: 'Continue from the wrong vendor identity kind.',
    vendorSession: { kind: 'claude-session', id: 'session-wrong-kind' },
  })
  assert.equal(wrongResumeIdentity.ok, false, 'resume rejects vendor-session identity from another protocol')
  assert.equal(wrongResumeIdentity.code, 'resume_identity_required', 'cross-protocol resume identity fails closed')

  const unsupportedResume = externalRuntimePolicy.createExternalAgentRuntimeSessionRequest({
    descriptor: { ...externalRuntimeDescriptors[0], supportsResume: false },
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    operation: 'resume',
    instruction: 'Continue from the caller-supplied vendor session correlation.',
  ...
  assert.equal(unsupportedResume.ok, false, 'resume requires explicit descriptor support')
  assert.equal(unsupportedResume.code, 'resume_unsupported', 'unsupported resume fails visibly')

  const completedExternalOutcome = (request, vendorSession, overrides = {}) => ({
    ...
    status: 'completed',
    vendorSession,
    output: 'External runtime completed with bounded evidence.',
    completedAt: 2000000003000,
    ...overrides,
  })
  const admittedExternalOutcome = externalRuntimePolicy.admitExternalAgentRuntimeSessionOutcome(
    externalOpen.request,
    completedExternalOutcome(externalOpen.request, codexThreadCorrelation),
  )
  assert.equal(admittedExternalOutcome.ok, true, 'matching external runtime outcomes are admitted')
  assert.equal(admittedExternalOutcome.outcome.status, 'completed', 'admitted external outcome preserves terminal status')
  assert.equal(Object.isFrozen(admittedExternalOutcome.outcome), true, 'admitted external outcomes are immutable')
  assert.equal(Object.isFrozen(admittedExternalOutcome.outcome.vendorSession), true, 'admitted vendor-session evidence is immutable')
  assert.deepEqual(Object.keys(admittedExternalOutcome.outcome).sort(), [
    'completedAt', 'descriptorId', 'operation', 'output', 'protocol', 'requestId', 'runtimeId', 'schema', 'status', 'taskId', 'vendorSession',
  ], 'external completed outcome v1 wire shape remains unchanged')

  const staleExternalOutcome = externalRuntimePolicy.admitExternalAgentRuntimeSessionOutcome(
    externalOpen.request,
    completedExternalOutcome(externalOpen.request, codexThreadCorrelation, { requestId: 'external-agent-session-stale' }),
  )
  assert.equal(staleExternalOutcome.ok, false, 'external outcomes cannot be replayed against another request identity')
  assert.equal(staleExternalOutcome.code, 'request_mismatch', 'stale external outcomes fail with request_mismatch')

  const changedResumeIdentityOutcome = externalRuntimePolicy.admitExternalAgentRuntimeSessionOutcome(
    externalResume.request,
    completedExternalOutcome(externalResume.request, { kind: 'codex-thread', id: 'thread-changed-by-runtime' }),
  )
  assert.equal(changedResumeIdentityOutcome.ok, false, 'resume outcomes cannot replace the supplied vendor-session correlation identity')
  assert.equal(changedResumeIdentityOutcome.code, 'session_mismatch', 'changed resume identity fails closed')

  const outcomeInjectionSecret = 'sk-external-outcome-injection-secret'
  const injectedExternalOutcome = externalRuntimePolicy.admitExternalAgentRuntimeSessionOutcome(
    externalOpen.request,
    completedExternalOutcome(externalOpen.request, codexThreadCorrelation, { credential: outcomeInjectionSecret }),
  )
  assert.equal(injectedExternalOutcome.ok, false, 'external outcomes reject unknown credential fields')
  assert.equal(JSON.stringify(injectedExternalOutcome).includes(outcomeInjectionSecret), false, 'outcome rejection never reflects injected credentials')

  const credentialExternalOutput = externalRuntimePolicy.admitExternalAgentRuntimeSessionOutcome(
    externalOpen.request,
    completedExternalOutcome(externalOpen.request, codexThreadCorrelation, { output: 'authorization=external-runtime-secret-value' }),
  )
  assert.equal(credentialExternalOutput.ok, false, 'external outcome admission rejects credential-shaped output')

  const admittedExternalFailure = externalRuntimePolicy.admitExternalAgentRuntimeSessionOutcome(
    externalOpen.request,
    {
      schema: EXTERNAL_AGENT_RUNTIME_SESSION_OUTCOME_SCHEMA,
      requestId: externalOpen.request.requestId,
      descriptorId: externalOpen.request.descriptorId,
      runtimeId: externalOpen.request.runtimeId,
      taskId: externalOpen.request.taskId,
      protocol: externalOpen.request.protocol,
      operation: externalOpen.request.operation,
      status: 'failed',
      error: { code: 'runtime_unavailable', message: 'The vendor runtime is unavailable.' },
      completedAt: 2000000003000,
    },
  )
  assert.equal(admittedExternalFailure.ok, true, 'bounded external runtime failures are admitted as terminal evidence')
  assert.equal(admittedExternalFailure.outcome.status, 'failed', 'admitted external failure remains distinct from policy rejection')

  let preAbortedPortCalls = 0
  const preAbortedController = new AbortController()
  preAbortedController.abort()
  const preAbortedExternalExecution = await externalRuntimePolicy.executeExternalAgentRuntimeSession({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    request: externalOpen.request,
    ...
    port: {
      open: async () => {
        preAbortedPortCalls += 1
        return completedExternalOutcome(externalOpen.request, codexThreadCorrelation)
      },
      resume: async () => {
        preAbortedPortCalls += 1
        return completedExternalOutcome(externalResume.request, codexThreadCorrelation)
      },
    },
  })
  assert.equal(preAbortedExternalExecution.ok, false, 'pre-aborted external sessions fail before dispatch')
  assert.equal(preAbortedExternalExecution.code, 'cancelled', 'pre-aborted external sessions report cancellation')
  assert.equal(preAbortedPortCalls, 0, 'pre-aborted external sessions never call the injected port')

  let successfulExternalPortCalls = 0
  const externalExecutionController = new AbortController()
  const successfulExternalExecution = await externalRuntimePolicy.executeExternalAgentRuntimeSession({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    request: externalOpen.request,
    signal: externalExecutionController.signal,
    port: {
      open: async (request, signal) => {
        successfulExternalPortCalls += 1
        assert.equal(request.requestId, externalOpen.request.requestId, 'external port receives the exact admitted request identity')
        assert.equal(signal, externalExecutionController.signal, 'external port receives the exact caller-supplied AbortSignal')
        return completedExternalOutcome(request, codexThreadCorrelation)
      },
      resume: async () => {
  ...
  assert.equal(successfulExternalExecution.ok, true, 'an injected external port can return an admitted bounded outcome')
  assert.equal(successfulExternalPortCalls, 1, 'an external open request invokes exactly one port operation')

  let postAbortedPortCalls = 0
  const postAbortedController = new AbortController()
  const postAbortedExternalExecution = await externalRuntimePolicy.executeExternalAgentRuntimeSession({
    descriptor: externalRuntimeDescriptors[0],
    runtime: externalDesktopRuntime,
    task: externalCodexCliStartedTask.task,
    request: externalOpen.request,
  ...
  assert.equal(postAbortedExternalExecution.ok, false, 'cancellation observed after the port returns suppresses outcome admission')
  assert.equal(postAbortedExternalExecution.code, 'cancelled', 'post-dispatch cancellation remains terminal')
  assert.equal(postAbortedPortCalls, 1, 'post-dispatch cancellation does not repeat the external effect')
